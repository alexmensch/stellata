...
- **AT-HYG v3.3** (stellar catalogue): https://codeberg.org/astronexus/athyg
  — maintained by David Nash. The one input is the frozen
  `data/athyg/inherited-spine.tsv`, read for its record of AT-HYG's merge
  decisions — which designations name one star, and which Gaia source it
  bound — that no primary supplies. Membership is the primaries-derived
  manifest at `data/membership/membership-manifest.tsv`, which the record
  build walks; every field on a record is sourced from a first-order
  catalogue this build pulls itself. Licence CC-BY-SA-4.0 follows the
...
- **Classic-designation cross indexes** (HD / HR / Bayer / Flamsteed /
  Gliese), the identifier spine and part of the membership term it
  derives from — four frozen
...
docs/science-catalog-ingestion.md      The merge AT-HYG performed,
                                        ...
                                        durable contract for membership,
                                        identifiers and per-field sourcing
                                        is docs/catalog-driver.md.
