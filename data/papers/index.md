...
One entry per cited work, under an explicit anchor that fixes its key. The
entry's **label** is its heading text before the dash — first author and
year — and the tree cites it by label and key
([How the tree cites a work](README.md#how-the-tree-cites-a-work)); the
entry is the only place its journal and identifiers live.

**Copy** names the version of the private copy that was read, and so what a
page number means
([The copy's version decides what a page number means](README.md#the-copys-version-decides-what-a-page-number-means)):
`image-only scan` when the copy has no text layer and its rows were read from
the page images, and `PDF page n is printed page X + n` (or `n − X`) when the
copy's page count is offset from the printed numbers. Otherwise it says why no
copy is held. **Identification** appears only when the work is not plainly
identified (`book`, `ambiguous`, `mismatch`, `not_found`); a **Note** records
what else identification found — an erratum, a near-namesake.

Each claims row records the paper, never the codebase: the claim as the paper
makes it, its own value, the page, and the passage quoted verbatim. **Status**
is `verified` (quoted from the copy) or `unverified` (no readable copy). Where
the codebase departs from a paper, the site that holds the value says so
([An entry records the paper, never the codebase](README.md#an-entry-records-the-paper-never-the-codebase);
[When the codebase departs from a paper](README.md#when-the-codebase-departs-from-a-paper)).

## Entries

<a id="abdurrouf2022"></a>
### Abdurro'uf 2022 — The Seventeenth Data Release of the Sloan Digital Sky Surveys: Complete Release of MaNGA, MaStar, and APOGEE-2 Data
...
|---|---|---|---|---|
| SDSS DR17: the complete release of MaNGA, MaStar and APOGEE-2 data |  | verified | p. 1 | “THE SEVENTEENTH DATA RELEASE OF THE SLOAN DIGITAL SKY SURVEYS: COMPLETE RELEASE OF MANGA, MASTAR AND APOGEE-2 DATA” |

<a id="andrae2023"></a>
### Andrae 2023 — Gaia Data Release 3
...
<a id="archinal2011"></a>
### Archinal 2011 — Report of the IAU Working Group on Cartographic Coordinates and Rotational Elements: 2009
...
<a id="archinal2018"></a>
### Archinal 2018 — Report of the IAU Working Group on Cartographic Coordinates and Rotational Elements: 2015
...
### Arzoumanian 2011 — Characterizing interstellar filaments with Herschel in IC 5146
...
### Arzoumanian 2019 — Characterizing the properties of nearby molecular filaments observed with Herschel
...
<a id="bahcall1980"></a>
### Bahcall 1980 — The universe at faint magnitudes. I - Models for the galaxy and the predicted star counts
...
### Bailer-Jones 2021 — Estimating Distances from Parallaxes. V. Geometric and Photogeometric Distances to 1.47 Billion Stars in Gaia Early Data Release 3
...
| Claim | Value | Status | Page | Passage |
|---|---|---|---|---|
| Bayesian geometric and photogeometric distance posteriors for Gaia EDR3 sources |  | verified | p. 1 (abstract); p. 3 | “The second, photogeometric, additionally uses the colour and apparent magnitude of a star” |
...
| Claim | Value | Status | Page | Passage |
|---|---|---|---|---|
| B-V -> Teff relation | Teff = 4600 (1/(0.92(B-V)+1.7) + 1/(0.92(B-V)+0.62)) K | verified | p. 4, eq. 14 | “it can be use to estimate the temperature from the widely used color index B − V … After this fine tuning, the final result is:” |
| B-V -> Teff relation derived from black-body colour temperatures (eq. 14: eq. 12 with γ fine-tuned), reported comparable to published stellar relations; no validity range stated | Teff = 4600 (1/(0.92(B-V)+1.7) + 1/(0.92(B-V)+0.62)) | verified | p. 4, eq. (14) | “After this fine tuning, the final result is: … comparable to other results in the literature (for a com- … parison, see [8] where B − V vs. T data for different stars … are represented together with several fittings)” |
...
| Claim | Value | Status | Page | Passage |
|---|---|---|---|---|
| δ Cep HST absolute parallax | π_abs = 3.66 ± 0.15 mas | verified | p. 2 (abstract); p. 10 | “we find πabs = 3.66 ± 0.15 mas” |
...
| Claim | Value | Status | Page | Passage |
|---|---|---|---|---|
| Johnson → Cousins (V−R) transformation, approximated by two linear relations split at (V−R)J = 1.0 | (V−R)c = 0.73 (V−R)J − 0.03 below (V−R)J = 1.0; (V−R)c = 0.62 (V−R)J − 0.08 for 1.0 < (V−R)J < 1.7 | verified | p. 591 | “The (V−R) … transformation (not plotted) can be approximated by two linear relations” |
| (V−R) Johnson → Cousins relation for (V−R)J < 1.0 | (V−R)c = 0.73 (V−R)J − 0.03 | verified | p. 591 | “{V-R)c = 0.73 (V-fí^ - 0.03,” |

<a id="blandhawthorn2016"></a>
### Bland-Hawthorn 2016 — The Galaxy in Context: Structural, Kinematic, and Integrated Properties
...
| magnitudes vs colour indices mutually inconsistent, from Milky Way analogues | note b gives no size; the ~0.1 mag is note a's typical calibration error. Table 2 itself differs by 0.06 in B−V (0.67 from the magnitudes vs 0.73 tabulated) and 0.16 in U−V (0.70 vs 0.86) | verified | p. 534, Table 2 notes | “The sdss and Johnson photometry are calibrated (typical errors ∼ 0.1 mag) with respect to the AB and Vega magnitude systems, respectively; … Different calibration schemes are needed for the sdss total magnitudes and the unbiassed galaxy colours which leads to inconsistencies” |
...
<a id="bodhaine1999"></a>
### Bodhaine 1999 — On Rayleigh Optical Depth Calculations
...
| Claim | Value | Status | Page | Passage |
|---|---|---|---|---|
| Earth sea-level Rayleigh optical depth at 650/550/450 nm | [0.049, 0.097, 0.221] (Table 3, sea level, 45° N: 4.9188E−02 at 0.650 µm, 9.7069E−02 at 0.550, 2.2111E−01 at 0.450; eq. 30 fits them) | verified | p. 1860, Table 3; p. 1859, eq. 30 | “Scattering cross section (per molecule) and Rayleigh optical depth (tR) for dry air containing 360 ppm CO2 … Best fits for the other two examples provided in Table 3 are” |
| blue/red Rayleigh ratio (450/650 nm) | 4.50 (derived: Table 3's 2.2111E−01 / 4.9188E−02) | verified | p. 1860, Table 3 | “Scattering cross section (per molecule) and Rayleigh optical depth (tR) for dry air containing 360 ppm CO2” |

<a id="bohlin1978"></a>
### Bohlin 1978 — A survey of interstellar H I from L-alpha absorption measurements. II
...
| Claim | Value | Status | Page | Passage |
|---|---|---|---|---|
| N_H/E(B-V) | 5.8×10²¹ cm⁻² mag⁻¹ | verified | p. 132 (abstract) | “A definitive value is found for the mean ratio of total neutral hydrogen to color excess,” |
| N_H per A_V (derived at R_V = 3.1) | 1.87×10²¹ cm⁻² mag⁻¹ (derived: 5.8×10²¹ / 3.1) | verified | p. 132 (abstract) | “A definitive value is found for the mean ratio of total neutral hydrogen to color excess,” |

<a id="bond2018"></a>
### Bond 2018 — Hubble Space Telescope Trigonometric Parallax of Polaris B, Companion of the Nearest Cepheid
...
<a id="bruzual2003"></a>
### Bruzual 2003 — Stellar population synthesis at the resolution of 2003
...
| Claim | Value | Status | Page | Passage |
|---|---|---|---|---|
| M/L_V of a simple stellar population, Chabrier IMF, Z = 0.02, 10 Gyr | 3.15 | verified | p. 8, Sect. 3 (model definition) | “mass-to-light ratio M/LV of simple stellar populations” |
| B−V of the same simple stellar population | 0.9574 | verified | p. 8, Sect. 3 (model definition), Fig. 1 caption | “Figure 1. Evolution of the B −V and V −K colours and stellar mass-to-light ratio M/LV of simple stellar populations for different metallicities, Z = 0.004 (dotted line), Z = Z⊙ = 0.02 (solid line)” |
...
| evolutionary tracks | Padova 1994 (Sect. 2.1, Table 1) | verified | p. 3, Sect. 2.1 | “We refer to this set of tracks as the ‘Padova 1994 library’.” |

<a id="butkevich2014"></a>
### Butkevich 2014 — Rigorous treatment of barycentric stellar motion
...
| A(K)/A(V) at R_V = 3.1 (Table 3, K, x = 0.46 µm⁻¹), from the infrared law a(x) = 0.574x^1.61, b(x) = −0.527x^1.61 (eq. 2) | 0.114 | verified | p. 249, eq. 2; p. 249, Table 3 | “The Infrared and Optical … K 0.46 0.1615 −0.1483 0.114” |
| A(λ)/A(V) at R_V = 3.1 in R, V and B (Table 3) | R 0.751, V 1.000, B 1.337 | verified | p. 249, Table 3 | “R 1.43 0.8686 -0.3660 0.751 ... B 2.27 0.9982 1.0495 1.337” |

<a id="casagrande2018"></a>
### Casagrande 2018 — On the use of Gaia magnitudes and new tables of bolometric corrections
...
<a id="chapman2009"></a>
### Chapman 2009 — The Mid-Infrared Extinction Law in the Ophiuchus, Perseus, and Serpens Molecular Clouds
...
| the larger-grain WD01 model that matches the law at A_Ks ≳ 1 | R_V = 5.5 | verified | p. 1 (abstract) | “the data are more consistent with the Weingartner & Draine RV = 5.5 model that uses larger maximum dust grain sizes” |

<a id="cioni2000"></a>
### Cioni 2000 — The tip of the red giant branch and distance of the Magellanic Clouds: results from the DENIS survey
...
<a id="ciotti1999"></a>
### Ciotti 1999 — Analytical properties of the R^(1/m) luminosity law
...
| Claim | Value | Status | Page | Passage |
|---|---|---|---|---|
| Sérsic b_n asymptotic expansion | b_n = 2n - 1/3 + 4/(405n) | verified | p. 5, eq. 18 | “Therefore, the first terms of the asymptotic expansion of b(m) (for real m) are … (C89, C91, PS97); note that 4/405 = 0.0098765” |

<a id="cook2023"></a>
### Cook 2023 — Completeness of the NASA/IPAC Extragalactic Database (NED) Local Volume Sample
...
<a id="corbelli2014"></a>
### Corbelli 2014 — Dynamical signatures of a ΛCDM-halo and the distribution of the baryons in M 33
...
| M33 disc orientation from tilted-ring fit | inner rings i ≈ 52–58°, PA ≈ 18–22° out to ~8 kpc, PA falling beyond (read off the Fig. 3 plot; the text prints no number) | verified | p. 8, Fig. 3 | “The inclination and position angle of the 11 free tilted rings used for deconvolving the 21-cm data.” |
...
### Costa-Almeida 2021 — M dwarf spectral indices at moderate resolution: accurate Teff and [Fe/H] for 178 southern stars
...
<a id="courteau2011"></a>
### Courteau 2011 — THE LUMINOSITY PROFILE AND STRUCTURAL PARAMETERS OF THE ANDROMEDA GALAXY
...
| M31 bulge and disc light fractions from the 2D bulge + disc fit (3.6 µm) | bulge 29 %, disc 71 %; B/D = 0.41 | verified | p. 24, Sect. 7; p. 2 | “yield light fractions for the bulge and disk equal to 29% and 71% respectively, for a B/D ratio of 0.41.” |
...
| horizontal illuminance 400 lx at 0 deg (geometric terminator) / 3.4 lx at 6 deg solar depression (civil twilight) |  | unverified |  |  |

<a id="creevey2023"></a>
### Creevey 2023 — Gaia Data Release 3
...
| Coarse spectral-type enum (spectraltype_esphs) written by ESP-ELS | O, B, A, F, G, K, M, CSTAR — from ESP-ELS (the archive column also carries "unknown"; the paper does not list it for this field) | verified | p. 16, Sect. 6.1.3 and fn. 16; p. 7, Sect. 3.7 | “ESP-ELS provides for 218 million targets with G ≤ 17.65 one of the following spectral type tags spectraltype_esphs” |
| spectraltype_esphs is a letter-only enum written by ESP-ELS | CSTAR, M, K, G, F, A, B, O | verified | p. 16, Sect. 6.1.3 and fn. 16 | “ESP-ELS provides for 218 million targets with G ≤ 17.65 one of the following spectral type tags spectraltype_esphs” |

<a id="cutri2003"></a>
### Cutri 2003 — The 2MASS All-Sky Catalog of Point Sources
...
| Claim | Value | Status | Page | Passage |
|---|---|---|---|---|
| PSC positions are right ascension and declination at equinox J2000 | RAdeg, DEdeg (J2000) | verified | l. 103-104 | “RAdeg (ra) Right ascension (J2000) ... DEdeg (dec) Declination (J2000)” |
| each PSC source carries the Julian date of its own measurement | JD (jdate), accurate to ±30 s | verified | l. 146; l. 485 | “JD (jdate) Julian date of source measurement” |

<a id="deau2013"></a>
### Déau 2013 — The opposition effect in Saturn's main rings as seen by Cassini ISS: 1. Morphology of phase functions and dependence on the local optical depth
...
| Claim | Value | Status | Page | Passage |
|---|---|---|---|---|
| Opposition-surge half-width (HWHM) where the local optical depth τ > 1.5 | 0.2° for τ > 1.5 (by optical depth, not by ring; the abstract gives 0.25° for τ > 0.7) | verified | p. 23, Sect. 5; p. 37, Fig. 5 | “A and HWHM stabilize to constant values of 1.7 and 0.2o, respectively, for τ > 1.5.” |
...
| Surge amplitude, A ring | ~1.4 | verified | p. 19, Sect. 3.1.1 | “The A Ring’s surge has, on average, an amplitude of about 1.4 (with a wide range of values)” |
...
| Claim | Value | Status | Page | Passage |
|---|---|---|---|---|
| LMC integrated (B−V) | 0.51 ± 0.02 | verified | p. 574, Abstract | “The integrated magnitudes and colors of the regions brighter than 25.3 mag/sec2 (B) are LMC: B = 0.94 ± 0.03, … +0.51 ± 0.02 in 100 square degrees” |
...
| Claim | Value | Status | Page | Passage |
|---|---|---|---|---|
| Galaxy total absolute B magnitude | M_B = −20.2 ± 0.15 | verified | p. 451, Abstract | “The following "most probable" values and their mean errors are adopted from a variety of recent determinations: morphological type of galaxy, SAB(rs)bc (T = 4 ± 0.5), … 8.5 ± 0.5 kpc, … −20.2 ± 0.15, … = 0.53 ± 0.04” |
| Galaxy integrated colour, carrying M_B to V | (B−V) = 0.53 | verified | p. 451, Abstract | “The following "most probable" values and their mean errors are adopted from a variety of recent determinations: morphological type of galaxy, SAB(rs)bc (T = 4 ± 0.5), … 8.5 ± 0.5 kpc, … −20.2 ± 0.15, … = 0.53 ± 0.04” |

<a id="devaucouleurs1991"></a>
### de Vaucouleurs 1991 — Third Reference Catalogue of Bright Galaxies (RC3)
...
- **Note:** The printed three-volume book is not held. RC3 tabulates B_T and (B−V)_T, not V_T

| Claim | Value | Status | Page | Passage |
|---|---|---|---|---|
| M31 (NGC 224) total magnitude and colour, B_T and (B−V)_T | V_T = B_T − (B−V)_T = 3.44 mag | verified | l. 152, l. 165 (VII/155 ReadMe); rc3 row NGC 224 | “BT (total B magnitude) ... (B-V)T (total (B-V))” |
| M33 (NGC 598) total magnitude and colour, B_T and (B−V)_T | V_T = B_T − (B−V)_T = 5.72 mag | verified | l. 152, l. 165 (VII/155 ReadMe); rc3 row NGC 598 | “BT (total B magnitude) ... (B-V)T (total (B-V))” |

<a id="dolan2002"></a>
### Dolan 2002 — A Photometric Study of the Young Stellar Population throughout the lambda Orionis Star-Forming Region
...
| Claim | Value | Status | Page | Passage |
|---|---|---|---|---|
| λ Ori ring of dense molecular gas and dust around the OB knot | 40 pc diameter | verified | p. 4, Sect. 1 | “The star-forming complex contains a tight knot of OB stars encircled by a 40 pc diameter ring of dense molecular gas and dust” |

<a id="dommanget1994"></a>
### Dommanget 1994 — Catalogue of the Components of Double and Multiple stars (CCDM), first edition
...
| Claim | Value | Status | Page | Passage |
|---|---|---|---|---|
| CCDM tied to the Hipparcos Input Catalogue (HIC) |  | verified | l. 15-17 | “the fundamental ties between the CCDM and the HIPPARCOS INPUT CATALOGUE (HIC)” |

<a id="drimmel2001"></a>
### Drimmel 2001 — Three‐dimensional Structure of the Milky Way Disk: The Distribution of Stars and Dust beyond 0.35R⊙
...
| Claim | Value | Status | Page | Passage |
|---|---|---|---|---|
| dust disc radial scale length (Table 1) | h_r = 2.26 ± 0.16 kpc | verified | p. 35, Table 1; p. 1, Abstract | “scale length hr 2.26 kpc 0.16” |
| dust disc vertical profile and base scale height (eqs. 13–14, Table 1) | ρ = ρ0 exp(−r/h_r) sech²(z/h_d); h_0,d = 134.4 ± 8.5 pc, flaring 14.8 pc/kpc | verified | p. 8, eqs. 13-14; p. 35, Table 1 | “ρaxi = ρ0 exp(−r/hr) sech2(z/hd)” … “base scale height h0,d 134.4 pc 8.5” |

<a id="ducati2001"></a>
### Ducati 2001 — Intrinsic Colors of Stars in the Near-Infrared
...
| Johnson V−R column of the V−R → V−Rc pairs (as tabulated by STScI) | 0.41–0.97 (G0–K5) | verified | p. 11, Table 3 | “INTRINSIC COLORS OF MAIN-SEQUENCE STARS ... G0 ... 0.41 ... K5 ... 0.97” |

<a id="dupuy2023"></a>
### Dupuy 2023 — Dynamic cosmography of the local Universe: Laniakea and five more watershed superclusters
...
<a id="dyudina"></a>
### Dyudina 2005 — Phase Light Curves for Extrasolar Jupiters and Saturns
...
### Edenhofer 2024 — A parsec-scale Galactic 3D dust map out to 1.25 kpc from the Sun
...
- **Copy:** `submittedVersion`
- **Note:** The paper gives the map's Zenodo DOI as 10.5281/zenodo.8187942 (pp. 7, 12, 24), the concept DOI covering all versions; 8187943 is its v1.0 record
...
| factor carrying the unitless ZGR23 extinction to Johnson V at 540.0 nm | 2.8 | verified | p. 7, Sect. 5 | “we adopted the extinction curve published in ZGR23 and multiplied the unitless ZGR23 extinction by a factor of 2.8.” |
| Posterior mean map plus 12 posterior samples released | posterior mean and 12 posterior samples | verified | p. 1, Abstract | “We generated 12 samples from the variational posterior of the 3D dust distribution and release the samples alongside the mean 3D dust map” |
| aim: a 3D map of interstellar dust extinction out to 1.25 kpc | ≤ 1.25 kpc | verified | p. 1, Abstract | “We aim to construct a new 3D map of the spatial distribution of interstellar dust extinction out to a distance of 1.25 kpc” |
...
| (B−V)T range over which the VT → V reduction was derived | −0.2 < (B−V)T < 1.8 | verified | p. 57, Sect. 1.3 Appendix 4; p. 142, Sect. 2.2 | “the following approximate linear transformations were derived between the two systems over the range −0.2 < (B − V)T < 1.8” |
...
| Hipparcos Catalogue fields: Johnson V (H5), B−V (H37), CCDM (H55), MultFlag (H59), HD (H71) |  | verified | p. 107, Field H5; p. 115, Field H37; p. 124, Field H55; p. 126, Field H59; p. 133, Field H71 | “The magnitude, V, in the Johnson UBV photometric system.” |
...
<a id="espenak2006"></a>
### Espenak 2006 — Five Millennium Canon of Solar Eclipses: -1999 to +3000 (NASA/TP-2006-214141)
...
| Claim | Value | Status | Page | Passage |
|---|---|---|---|---|
| ΔT polynomial set, −1999 to +3000 (every coefficient and interval boundary) | eqs. 11–25, one polynomial per interval | verified | p. 14–16, Sect. 2.7, eqs. 11–25 | “a series of polynomial expressions have been created to simplify the evaluation of ∆T for any time during the interval –1999 to +3000.” |
...
<a id="espenak2009"></a>
### Espenak 2009a — Five Millennium Catalog of Solar Eclipses: -1999 to +3000 (NASA/TP-2009-214174)
...
| Claim | Value | Status | Page | Passage |
|---|---|---|---|---|
| Solar eclipse catalogue rows (ΔT, type, γ, magnitude, greatest-eclipse lat/lon, path width) | Appendix A rows, e.g. cat. no. 3379 (−0584 May 28) and 9546 (2017 Aug 21) | verified | Appendix A, e.g. p. A-59 (cat. no. 3379, −0584 May 28), p. A-162 (cat. no. 9546, 2017 Aug 21) | “3379 169 -0584 May 28 19:28:50 18384 ... T n- 0.3201 1.0798 38.2N 45.0W 71 158 271” |
...
| Catalog uses the same ΔT polynomial set and −0.000012932(y−1955)² correction |  | verified | p. iv, Preface; p. 6, eq. 1-1 and Table 1-2; p. 12, Sect. 2.7; p. 14, eq. 2-19 | “The Canon and the Catalog both use the same solar and lunar ephemerides as well as the same values of ∆T.” |

<a id="espenak2009lunar"></a>
### Espenak 2009b — Five Millennium Catalog of Lunar Eclipses: -1999 to +3000 (NASA/TP-2009-214173)
...
| Claim | Value | Status | Page | Passage |
|---|---|---|---|---|
| Lunar eclipse catalogue rows (ΔT, type, γ, umbral magnitude, greatest-eclipse lat/lon) | Appendix A rows, e.g. −1982 Jul 18, −0583 May 03, 1938 Nov 07, 2080 Apr 04, 2912 Jul 20 | verified | p. A-159 (1938 Nov 07); the other spot-checked rows on p. A-1, p. A-57, p. A-164, p. A-198 | “9509 476 1938 Nov 07 22:26:42 24 -757 125 T -p 0.2738 2.3585 1.3525 ... 17N 19E” |

<a id="esposito2002"></a>
### Esposito 2002 — Planetary rings

Rep. Prog. Phys. 65, 1741-1783 (2002) · [doi:10.1088/0034-4885/65/12/201](https://doi.org/10.1088/0034-4885/65/12/201)
...
### Fabricius 2002a — Henry Draper catalogue identifications for Tycho-2 stars
...
### Fabricius 2002b — The Tycho double star catalogue

Fabricius C., Høg E., Makarov V. V., Mason B. D., Wycoff G. L. & Urban S. E.; A&A 384, 180-189 (2002) · [doi:10.1051/0004-6361:20011822](https://doi.org/10.1051/0004-6361:20011822) · [2002A&A...384..180F](https://ui.adsabs.harvard.edu/abs/2002A%26A...384..180F)
...
| TDSC positions, proper motions and BT/VT photometry | 66,219 components | verified | p. 181, Sect. 1 | “It presents accurate positions, proper motions, BT and VT photometry for 66 219 components” |

<a id="federrath2010"></a>
### Federrath 2010 — Comparing the statistics of interstellar turbulence in simulations and observations
...
### Federrath 2013 — On the Star Formation Efficiency of Turbulent Magnetized Clouds
...
<a id="flynn2006"></a>
### Flynn 2006 — On the mass-to-light ratio of the local Galactic disc and the optical luminosity of the Galaxy
...
| Claim | Value | Status | Page | Passage |
|---|---|---|---|---|
| disc central surface brightness, nearly constant across spirals (Freeman's law) | B(0)c = 21.65 ± 0.30 (σ) mag/arcsec², B band | verified | p. 818, Sect. III; p. 811, Abstract | “B{0)c is nearly constant at B(0)c = 21.65 ± 0.30(0-) mag per square second of arc” |

<a id="french1986"></a>
### French 1986 — Structure of the Uranian Rings. II. Ring Orbits and Widths
...
<a id="french1988"></a>
### French 1988 — Uranian ring orbits from earth-based and Voyager occultation observations
...
<a id="french1991"></a>
### French 1991 — Dynamics and structure of the Uranian rings
...
- **Copy:** unobtainable — University of Arizona Press 1991 chapter; no digital or print copy reachable
- **Note:** CSL authors: French, Nicholson, Porco, Marouf — the fourth author is Marouf, not Elliot; bibcode 1991uran.book..327F did not resolve via ADS link gateway (may be valid but lacks a full-text link)
...
<a id="frey1974"></a>
### Frey 1974 — Studies of the Major Planet Satellite Systems

NASA Goddard Space Flight Center report X-922-74-112 (April 1974); NTRS 19740014371

- **Copy:** `publishedVersion` — PDF page n is printed page n − 5
- **Identification:** book — NASA NTRS record: Frey, H.; Lowman, P. D.; 1974-04-01

| Claim | Value | Status | Page | Passage |
|---|---|---|---|---|
| satellite B-V / V-R (Table IV): Io, Europa, Ganymede, Callisto, Dione, Rhea, Titan, Triton | Io 1.17/0.66, Europa 0.87/0.57, Ganymede 0.83/0.59, Callisto 0.86/0.61, Dione 0.71/0.48, Rhea 0.76/0.61, Titan 1.29/0.84, Triton 0.77/0.58 | verified | p. 14, Table IV (unnumbered page between printed -13- and -15-) | “S VI Titan 8.3 0.75 1.29 0.84 0.11 -1.21 ... N I Triton 15.6 0.40 0.77 0.58 0.44 -1.23” |
| filter effective wavelengths (Table III), R at 0.69 um | U .35, B .45, V .55, R .69, I .82 um | verified | p. 13, Table III | “TABLE III. WAVELENGTHS OF U B V R I FILTERS ... .35 .45 .55 .69 .82” |
| coverage: B-V but no V-R for Enceladus, Tethys, Iapetus; nothing for Mimas; Titania and Oberon both indices; no lunar row |  | verified | p. 14, Table IV (unnumbered page between printed -13- and -15-) | “S I Mimas 12.1 -- -- -- -- +2.5; S II Enceladus 11.7 -- 0.62 -- --” |

<a id="gaiacollab2018cat"></a>
### Gaia Collaboration 2018 — Gaia DR2 catalogue (VizieR record)
...
| Claim | Value | Status | Page | Passage |
|---|---|---|---|---|
| bibcode of the Gaia DR2 catalogue record (VizieR I/345) | 2018yCat.1345....0G | verified | l. 7 | “=2018yCat.1345....0G” |

<a id="gaiacollab2022cat"></a>
### Gaia Collaboration 2022 — Gaia DR3 catalogue (VizieR record)
...
| Claim | Value | Status | Page | Passage |
|---|---|---|---|---|
| bibcode of the Gaia DR3 catalogue record (VizieR I/355) | 2022yCat.1355....0G | verified | l. 6 | “=2022yCat.1355....0G” |

<a id="gaiadr3doc"></a>
### Carrasco 2022 — Gaia DR3 documentation, Sect. 5.5.1: Photometric relationships with other photometric systems
...
<a id="gieren2013"></a>
### Gieren 2013 — THE ARAUCARIA PROJECT. A DISTANCE DETERMINATION TO THE LOCAL GROUP SPIRAL M33 FROM NEAR-INFRARED PHOTOMETRY OF CEPHEID VARIABLES
...
<a id="gliese1991"></a>
### Gliese 1991 — Preliminary Version of the Third Catalogue of Nearby Stars
...
| B-V colour |  | verified | l. 81 | “76- 80  F5.2   mag     B-V      ? color” |
| code on the resulting parallax (n_plx): trigonometric or photometric |  | verified | l. 90-94, l. 150-155 | “plx      ? Resulting parallax ... n_plx     *[rwsop] Code on plx” |
| B1950 position and total proper motion |  | verified | l. 65-73 | “Right Ascension B1950 (hours) ... pm       ? Total proper motion” |
| n_plx code r: parallax from spectral types and broad-band colours | r | verified | l. 92-94, l. 151; catalog row Gl 423 A | “r    parallax from spectral types and broad-band colors” |

<a id="golovin2023"></a>
### Golovin 2023 — The Fifth Catalogue of Nearby Stars (CNS5)
...
| CNS5 object count | 5,931 objects, 5,230 of them stars | verified | p. 1, Abstract | “The CNS5 contains 5931 objects, including 5230 stars” |
...
| Claim | Value | Status | Page | Passage |
|---|---|---|---|---|
| PCRV: weighted mean absolute radial velocities of Hipparcos stars | 35,495 stars | verified | p. 1, Abstract | “The PCRV contains weighted mean absolute radial velocities for 35 495 Hipparcos stars” |

<a id="graczyk2020"></a>
### Graczyk 2020 — A Distance Determination to the Small Magellanic Cloud with an Accuracy of Better than Two Percent Based on Late-type Eclipsing Binary Stars
...
| Claim | Value | Status | Page | Passage |
|---|---|---|---|---|
| SMC eclipsing-binary distance | 62.44 kpc, mu = 18.977 | verified | p. 1, Abstract | “the SMC center of DSMC = 62.44 ± 0.47(stat.)±0.81 (syst.) kpc corresponding to a distance modulus (m − M )SMC = 18.977” |
...
| Claim | Value | Status | Page | Passage |
|---|---|---|---|---|
| M32 Sersic index n = 1.5 | n = 1.5 (the bulge component of a bulge + exponential-disc fit) | verified | p. 1, Abstract | “the best-fitting r1/n bulge model which has a Sérsic index n = 1.5” |
...
| Claim | Value | Status | Page | Passage |
|---|---|---|---|---|
| R0 (Sun to Galactic centre distance) | R0 = 8122 ± 31 pc | verified | p. 10, Table A.1 | “R0 8127 ± 31 8122 ± 31 pc” |

<a id="gray2003"></a>
### Gray 2003 — Contributions to the Nearby Stars (NStars) Project: Spectroscopy of Stars Earlier than M0 within 40 Parsecs: The Northern Sample. I.
...
| Claim | Value | Status | Page | Passage |
|---|---|---|---|---|
| Castor A (HIP 36850, HD 60178) spectral type | A1.5 IV+ | verified | p. 18, Table 1 | “36850 60178 A1.5 IV+” |

<a id="gurnett2013"></a>
### Gurnett 2013 — In Situ Observations of Interstellar Plasma with Voyager 1
...
<a id="gurnett2019"></a>
### Gurnett 2019 — Plasma densities near and beyond the heliopause from the Voyager 1 and 2 plasma wave instruments
...
- **Copy:** `publishedVersion` — online-first copy printing no page numbers; PDF page n is printed page 1023 + n

| Claim | Value | Status | Page | Passage |
|---|---|---|---|---|
| Voyager 2 heliopause crossing | 2018-11-05 | verified | p. 1024 (first page; the copy prints no page number) | “V2 tentatively reached the heliopause13–16 on 5 November 2018 at a heliocentric radial distance of 119.0 au” |

<a id="halbwachs2023"></a>
### Halbwachs 2023 — Gaia Data Release 3: Astrometric binary star processing
...
| Claim | Value | Status | Page | Passage |
|---|---|---|---|---|
| Gaia DR3 NSS two-body orbits with Thiele-Innes fits |  | verified | p. 7, Sect. 5.1 | “A, B, F, and G are the Thiele-Innes (TI) elements” |
...
<a id="hamann2006"></a>
### Hamann 2006 — The Galactic WN stars
...
<a id="hartkopf2001"></a>
### Hartkopf 2001 — The 2001 US Naval Observatory Double Star CD-ROM. II. The Fifth Catalog of Orbits of Visual Binary Stars

AJ 122, 3472 · [doi:10.1086/323921](https://doi.org/10.1086/323921)

- **Copy:** `publishedVersion`
- **Note:** The copy describes the Fifth Catalog (web edition, fn. 3: orb5.html), the published description of the USNO orbit-catalogue series; the Sixth Catalog (ORB6) postdates it

| Claim | Value | Status | Page | Passage |
|---|---|---|---|---|
| the Fifth Catalog continues the USNO series of visual binary orbit compilations |  | verified | p. 3472, Abstract | “The Fifth Catalog of Orbits of Visual Binary Stars continues the series of compilations of visual binary star orbits” |
...
<a id="hoffleit1991"></a>
### Hoffleit 1991 — The Bright Star Catalogue, 5th Revised Ed. (Preliminary Version)
...
| rows carrying an HD; the 14 HD-less entries are non-stellar | 9,096 stars; the other 14 are novae or extragalactic objects retained for the numbering (the ReadMe counts stars, not HD numbers) | verified | l. 35-37 | “14 objects catalogued in the original compilation of 1908 are novae or extragalactic objects” |

<a id="hog2000"></a>
### Høg 2000 — The Tycho-2 Catalogue
...
| sources of the Tycho-2 proper motions | Tycho-2, the Astrographic Catalogue and 143 other ground-based catalogues | verified | p. L27, Sect. 1 | “The proper motions are derived for 96 per cent of the stars from the observed positions in Tycho-2, the Astrographic Catalogue and 143 other ground-based catalogues” |

<a id="huchra2012"></a>
### Huchra 2012 — The 2MASS Redshift Survey — Description and Data Release
...
<a id="ibata1997"></a>
### Ibata 1997 — The Kinematics, Orbit, and Survival of the Sagittarius Dwarf Spheroidal Galaxy
...
| Sagittarius dSph half-brightness minor-axis diameter | 2 × 550 pc, at 25 kpc | verified | p. 7 | “the half-brightness minor axis diameter, for a distance of 25 kpc, is 2 × 550 pc” |

<a id="ignatiev2009"></a>
### Ignatiev 2009 — Altimetry of the Venus cloud tops from the Venus Express observations
...
| pressure and altitude of the main cloud top | about 40 mbar, at 68 km | verified | p. 1, Sect. 1 | “above the main cloud which top is located at about 40 mbar (68 km)” |

<a id="ireland2004"></a>
### Ireland 2004 — On the observability of geometric pulsation of M-type Mira variables
...
<a id="irwin2024"></a>
### Irwin 2024 — Modelling the seasonal cycle of Uranus's colour and magnitude, and comparison with Neptune
...
<a id="jonsson2020"></a>
### Jönsson 2020 — APOGEE Data and Spectral Analysis from SDSS Data Release 16
...
| Claim | Value | Status | Page | Passage |
|---|---|---|---|---|
| APOGEE DR16 radial velocities, the VHELIO AVG entry of the allStar file |  | verified | p. 11, Sect. 5.1 | “The radial velocities are provided in the VHELIO AVG entry in the allStar file.” |

<a id="juric"></a>
### Jurić 2008 — The Milky Way Tomography with SDSS. I. Stellar Number Density Distribution
...
| Claim | Value | Status | Page | Passage |
|---|---|---|---|---|
| Milky Way decomposition into an oblate halo, a disc component and localized overdensities |  | verified | p. 1, Abstract | “The data show strong evidence for a Galaxy consisting of an oblate halo, a disk component, and a number of localized overdensities.” |

<a id="kainulainen2009"></a>
### Kainulainen 2009 — Probing the evolution of molecular cloud structure: From quiescence to birth
...
| Claim | Value | Status | Page | Passage |
|---|---|---|---|---|
| UNGC: all-sky catalogue of nearby galaxies with distances within 11 Mpc or V_LG < 600 km/s | 869 galaxies | verified | p. 2, Abstract | “We present an all-sky catalog of 869 nearby galaxies, having individual distance estimates within 11 Mpc or corrected radial velocities VLG < 600 km s−1” |

<a id="kepler2007"></a>
### Kepler 2007 — White dwarf mass distribution in the SDSS
...
| Claim | Value | Status | Page | Passage |
|---|---|---|---|---|
| mean mass of SDSS DA white dwarfs brighter than g = 19 and hotter than 12,000 K | 0.593 ± 0.016 M☉ | verified | p. 1, Abstract | “The mean mass for the DA stars brighter than g=19 and hotter than Teff = 12 000 K is … ≃ 0.593 ± 0.016 M⊙” |

<a id="kersten2021"></a>
### Kersten 2021 — Controlled global Ganymede mosaic from Voyager and Galileo images
...
<a id="kervella2003"></a>
### Kervella 2003 — The interferometric diameter and internal structure of Sirius A
...
<a id="kilic2020"></a>
### Kilic 2020 — The 100 pc White Dwarf Sample in the SDSS Footprint
...
| Claim | Value | Status | Page | Passage |
|---|---|---|---|---|
| peak of the 100 pc white-dwarf mass distribution | 0.59 M☉ | verified | p. 1, Abstract | “mass distribution has an extremely narrow peak at 0.59 M” |

<a id="kim2014"></a>
### Kim 2014 — The Extended Virgo Cluster Catalog
...
| Claim | Value | Status | Page | Passage |
|---|---|---|---|---|
| EVCC: 1,589 galaxies, 676 of them not in the VCC; photometry for the 1,183 VCC galaxies not in common with it | 1,589 galaxies; 1,183 VCC-only | verified | p. 2, Abstract; p. 33, Sect. 5 | “Our selection process secured a total of 1589 galaxies of which 676 galaxies are not included in the VCC. … we present in Table 3 the photometric parameters for 1183 galaxies in the VCC which are not in common with the EVCC” |

<a id="kolbas2015"></a>
### Kolbas 2015 — Spectroscopically resolving the Algol triple system
...
| Claim | Value | Status | Page | Passage |
|---|---|---|---|---|
| Algol inner pair: a late B-type star and an early K-type subgiant filling its Roche lobe | late B + early K subgiant | verified | p. 4151, Sect. 2; p. 4159, Table 5 | “The inner pair consists of a late B-type star in orbit with an early K-type subgiant which fills its Roche lobe.” |
| Algol B (Aa2) radius and Teff | R = 3.43 ± 0.01 R☉, Teff = 4900 ± 300 K | verified | p. 4159, Table 5 | “Radius R R 2.87 ± 0.04 3.43 ± 0.01 1.7 ± 0.3 … Effective temperature Teff K 12 550 ± 120 4900 ± 300 7550 ± 250” |

<a id="kostjuk2002"></a>
### Kostjuk 2002 — HD-DM-GC-HR-HIP-Bayer-Flamsteed Cross Index
...
<a id="lacour2009"></a>
### Lacour 2009 — The Pulsation of chi Cygni Imaged by Optical Interferometry
...
### Lallement 2019 — Gaia-2MASS 3D maps of Galactic interstellar dust within 3 kpc
...
- **Copy:** `ADS scan of published article` — image-only scan: no text layer, rows read from the page images
- **Note:** No DOI

| Claim | Value | Status | Page | Passage |
|---|---|---|---|---|
| obliquity at −2950 | 24.0165° | verified | p. 68, Table 8; p. 69, eq. 35 | “The general accumulated precession pA and the obliquity εA are given in arcseconds and the time t is measured in units of 10000 julian years from J2000” |

<a id="leike2020"></a>
### Leike 2020 — Resolving nearby dust clouds
...
- **Copy:** `submittedVersion`
- **Note:** The abstract's 1 pc is the map's resolution; p. 4 (Sect. 6) recommends distrusting the smallest scales — only scales of 2 pc or higher are stable

| Claim | Value | Status | Page | Passage |
|---|---|---|---|---|
| dust map of the nearest clouds, to 400 pc |  | verified | p. 1, Abstract | “We reconstructed a highly resolved dust map, showing the nearest dust clouds at a distance of up to 400 pc with a resolution of 1 pc.” |
| map resolution, and the smallest scale it holds as stable | 1 pc; stable at ≥ 2 pc | verified | p. 1, Abstract; p. 4, Sect. 6 | “showing the nearest dust clouds at a distance of up to 400 pc with a resolution of 1 pc … only at scales of 2 pc or higher can the result be considered to be stable” |
...
- **Copy:** `publishedVersion` — PDF page n is printed page n − 1
...
| Table 24 integrated starlight at 0.55 µm, b = 30° | λI_λ = 577 × 10⁻⁹ W m⁻² sr⁻¹ | verified | p. 57 (printed p. 56), Table 24 | “Table 24. Surface brightness due to integrated starlight (given as λIλ , respectively νIν )” … “0.55 577 250” |
...
<a id="lemmon2015"></a>
### Lemmon 2015 — Dust aerosol, clouds, and the atmospheric optical depth record over 5 Mars years of the Mars Exploration Rover mission
...
| Claim | Value | Status | Page | Passage |
|---|---|---|---|---|
| Mars background dust tau 0.2-0.5 | τ = 0.2-0.5 (the text never states this range: aphelion-season lows of ~0.3 at Spirit and below ~0.5 at Opportunity, p. 22; the 0.2 floor is read off Fig. 7, p. 55; the text's only "0.2-0.5", p. 29, is a dust size-distribution variance) | verified | p. 22, Sect. 4.1; p. 55, Fig. 7 | “below ~0.3 by sol 155 (LS=45°), and remained similarly low until about sol 350 (LS=135°). During this time, Opportunity optical depths declined from 0.95 to below ~0.5.” |

<a id="lepine2005"></a>
### Lépine 2005 — A Catalog of Northern Stars with Annual Proper Motions Larger than 0.15 Seconds of Arc (LSPM Catalog – North)
...
| Claim | Value | Status | Page | Passage |
|---|---|---|---|---|
| LSPM-North: northern stars with annual proper motions larger than 0.15″ | > 0.15″/yr | verified | p. 1, Abstract | “A CATALOG OF NORTHERN STARS WITH ANNUAL PROPER MOTIONS LARGER THAN 0.15 SECONDS OF ARC” |

<a id="licquia2015"></a>
### Licquia 2015a — IMPROVED ESTIMATES OF THE MILKY WAY’S STELLAR MASS AND STAR FORMATION RATE FROM HIERARCHICAL BAYESIAN META-ANALYSIS
...
| stellar mass split M* / bulge / disc | 6.08 ± 1.14, 0.91 ± 0.07, 5.17 ± 1.11 × 10¹⁰ M☉ | verified | p. 1, Abstract | “their combination yields a total stellar mass of M⋆ = 6.08 ± 1.14 × … M⊙ (assuming a Kroupa IMF and an exponential disk profile)” |
| IMF of the mass estimates | Kroupa IMF | verified | p. 1, Abstract | “Kroupa initial mass function (IMF)” |

<a id="licquia2015b"></a>
### Licquia 2015b — Unveiling the Milky Way: A New Technique for Determining the Optical Color and Luminosity of Our Galaxy
...
| Claim | Value | Status | Page | Passage |
|---|---|---|---|---|
| Galactic colour and luminosity from Milky Way analogue galaxies matched to its stellar mass and star formation rate |  | verified | p. 1, Abstract | “by selecting a sample of Milky Way analog galaxies designed to reproduce the best Galactic M? and Ṁ? measurements” |
| Licquia's own Galaxy M_V | M_V = −21.51 | verified | p. 17 | “these compare well with our slightly brighter esti- … mates of 0MB = −20.84 … and 0MV = −21.51” |
...
<a id="limaneto1999"></a>
### Lima Neto 1999 — The specific entropy of elliptical galaxies: an explanation for profile-shape distance indicators?
...
<a id="lindal1983"></a>
### Lindal 1983 — The atmosphere of Titan: An analysis of the Voyager 1 radio occultation measurements
...
| Claim | Value | Status | Page | Passage |
|---|---|---|---|---|
| Titan surface temperature and pressure | 94.0 ± 0.7 K, 1496 ± 20 mbar | verified | p. 348, Abstract; p. 354 | “temperature and pressure at the surface of 94.0 ± 0.7°K and 1496 ± 20 mbar, respectively.” |

<a id="lindegren2021"></a>
### Lindegren 2021 — Gaia Early Data Release 3: Parallax bias versus magnitude, colour, and position
...
| Claim | Value | Status | Page | Passage |
|---|---|---|---|---|
| parallax zero-point bias depends on magnitude, colour and ecliptic latitude |  | verified | p. 1, Abstract | “The parallax bias is found to depend in a non-trivial way on (at least) the magnitude, colour, and ecliptic latitude of the source.” |

<a id="makarov2014"></a>
### Makarov 2014 — HyperLEDA. III. The catalogue of extragalactic distances
...
| Claim | Value | Status | Page | Passage |
|---|---|---|---|---|
| HyperLEDA compiles major and minor diameters and major-axis position angles |  | verified | p. 3, Sect. 2 | “Sizes and position angles: These catalogues compile information on major and minor diameters of objects as well as the position angle of the major axis” |
...
| Claim | Value | Status | Page | Passage |
|---|---|---|---|---|
| origin of the Saturn ring-tilt law | ΔV = −1.825·sin β + 0.026·α − 0.378·sin β·e^(−2.25·α), V₁(0) = −8.914 | verified | p. 58, Table 2; p. 57, eq. 2; p. 56 (β defined below eq. 1) | “b is the effective inclination of the ring system (specifically, the square root of the product of the saturnicentric latitude of the Sun and the saturnicentric latitude of the Earth, set to zero when the signs are opposite)” |

<a id="mallama2017"></a>
### Mallama 2017 — Comprehensive wide-band magnitudes and albedos for the planets, with applications to exo-planets and Planet Nine
...
| Claim | Value | Status | Page | Passage |
|---|---|---|---|---|
| planet B−V / V−Rc (Table 3) | B−V, V−Rc: Mercury 0.97/0.52, Venus 0.70/0.35, Earth 0.47/0.29, Mars 1.36/0.82, Jupiter 0.86/0.35, Saturn 1.07/0.51, Uranus 0.50/−0.27, Neptune 0.39/−0.33 (differences of each planet's Reference row, except Saturn's V−Rc 0.51, which is its Synthetic row, −9.08 − (−9.59); its Reference row gives 0.68) | verified | p. 9, Table 3 | “Table 3. Johnson-Cousins magnitudes … Saturn Photometric -7.08 -7.84 -8.91 -9.59 -9.61 … Synthetic -7.32 -8.02 -9.08 -9.76 -9.83 -9.59 -9.74” |
| Saturn photometric V (−8.91) and synthetic V (−9.08) disagree by 0.17 mag, so the Reference V−Rc (0.68) pairs photometric V with synthetic Rc; the Synthetic row alone gives 0.51 | 0.17 mag | verified | p. 9, Table 3 | “Saturn Photometric -7.08 -7.84 -8.91 -9.59 -9.61 ---- ---- … Synthetic -7.32 -8.02 -9.08 -9.76 -9.83 -9.59 -9.74” |
| no reference magnitude for Pluto |  | verified | p. 26 | “We did not establish reference magnitude for Pluto in this paper” |
...
| Moon not covered (planets only) |  | verified | p. 2, Abstract | “Complete sets of reference magnitudes in all 7 Johnson-Cousins bands (U, B, V, R, I, RC and IC) and the 5 principal Sloan bands (u’, g’, r’, i', and z’) are presented for the 8 planets.” |

<a id="mallamahilton2018"></a>
### Mallama 2018 — Computing apparent planetary magnitudes for The Astronomical Almanac
...
| Claim | Value | Status | Page | Passage |
|---|---|---|---|---|
| Saturn ring-tilt law (Eq. 10) | V₁(0) = −8.914; ΔV = −1.825·sin β + 0.026·α − 0.378·sin β·e^(−2.25·α); α < 6.5°, β < 27° | verified | p. 18, Eq. 10 | “used these latitude … values as indicated in Eq. 10 which applies to α < 6.5 and β < 27 for the planet and rings. … V = 5 log10 ( r d ) -8.914 - 1.825 sin β + 0.026 α - 0.378 sin β e” |
...
| Saturn globe V₁(0) = −8.95 (Eq. 11), zero-point delta 0.036 | −8.95 | verified | p. 19, Eq. 11; p. 18 | “V = 5 log10 ( r d ) - 8.95 - 3.7E-04 α + 6.16E-04 α … The V1(0) magnitude for Saturn was determined to be -8.95.” |
...
- **Note:** The table asks to be cited with Pecaut & Mamajek 2013 until it is republished

| Claim | Value | Status | Page | Passage |
|---|---|---|---|---|
| dwarf masses O3V–O9V (Msun column; no O0V–O2V rows) | 59, 48, 43, 35, 28, 23.6, 20.2 M☉ | verified | l. 24-34 | “z-Y  Msun  #SpT” … “O3V    44900 4.652” |
| dwarf masses B0V–B9V (Msun column) | 17.7, 11.8, 7.3, 5.4, 5.1, 4.7, 4.3, 3.92, 3.38, 2.75 M☉ | verified | l. 36-48 | “z-Y  Msun  #SpT” … “B0V    31400 4.497” |
| dwarf masses A0V–A9V (Msun column) | 2.18, 2.05, 1.98, 1.86, 1.93, 1.88, 1.83, 1.77, 1.81, 1.75 M☉ | verified | l. 50-59 | “z-Y  Msun  #SpT” … “A0V    9700  3.987” |
| dwarf masses F0V–F9V (Msun column) | 1.61, 1.50, 1.46, 1.44, 1.38, 1.33, 1.25, 1.21, 1.18, 1.13 M☉ | verified | l. 60-69 | “z-Y  Msun  #SpT” … “F0V    7220  3.859” |
| dwarf masses G0V–G9V (Msun column) | 1.06, 1.03, 1.00, 0.99, 0.985, 0.98, 0.97, 0.95, 0.94, 0.90 M☉ | verified | l. 71-80 | “z-Y  Msun  #SpT” … “G0V    5930  3.773” |
| dwarf masses K0V–K9V (Msun column) | 0.88, 0.86, 0.82, 0.78, 0.73, 0.70, 0.69, 0.64, 0.62, 0.59 M☉ | verified | l. 81-90 | “z-Y  Msun  #SpT” … “K0V    5270  3.723” |
| dwarf masses M0V–M9V (Msun column) | 0.57, 0.50, 0.44, 0.37, 0.23, 0.162, 0.102, 0.090, 0.085, 0.079 M☉ | verified | l. 91-109 | “z-Y  Msun  #SpT” … “M0V    3850  3.585” |
| dwarf M_V at subclasses 0/5/9 (Mv column; O at 5/9 only) | O5V −5.35, O9V −4.20; B0V −3.90, B5V −0.85, B9V 0.50; A0V 0.99, A5V 2.01, A9V 2.43; F0V 2.57, F5V 3.37, F9V 4.25; G0V 4.48, G5V 4.98, G9V 5.55; K0V 5.78, K5V 7.28, K9V 8.56; M0V 8.80, M5V 14.15, M9V 19.40 | verified | l. 26-109 | “#SpT   Teff  logT   BCv    logL   Mbol R_Rsun   Mv” … “M5V    3060  3.486 -3.11  -2.52  11.04  0.196  14.15” |

<a id="martins2005"></a>
### Martins 2005 — A new calibration of stellar parameters of Galactic O stars
...
| O6V log Q_H | 48.96 | verified | p. 15, Table 1 | “6 38151 3.92 -4.92 -3.57 5.30 10.23 31.73 24.15 23.39 48.96 48.19” |
...
| O9V log Q_H | 47.90 | verified | p. 15, Table 1 | “9 31524 3.92 -4.05 -3.01 4.72 7.73 18.03 23.34 21.21 47.90 45.77” |
| log Q0 for luminosity classes III and I, O5–O9 (Tables 2–3) | III: O5 49.44, O6 49.22, O7 49.00, O8 48.76, O9 48.42; I: O5 49.61, O6 49.44, O7 49.27, O8 49.03, O9 48.80 | verified | p. 15, Tables 2-3 | “Table 2. Same as Table 1 for luminosity class III stars.” … “Table 3. Same as Table 1 for luminosity class I stars.” |

<a id="mason2001"></a>
### Mason 2001 — The 2001 US Naval Observatory Double Star CD-ROM. I. The Washington Double Star Catalog
...
<a id="mccomas2015"></a>
### McComas 2015 — LOCAL INTERSTELLAR MEDIUM: SIX YEARS OF DIRECT SAMPLING BY IBEX
...
| Claim | Value | Status | Page | Passage |
|---|---|---|---|---|
| interstellar He flow direction, J2000 ecliptic (Table 3 working values) | (λ, β) = (75.7°, −5.1°) | verified | p. 8, Table 3 | “Working values” (1000 AU) 25.4 75.7 −5.1 7500” |
...
| NGC 205 position angle and ellipticity (Table 3) | PA = 28 ± 5°, ε = 0.43 ± 0.10 | verified | p. 101, Table 3 | “NGC 205 8.1 0.1 2.46 0.10 15.4: g 28 5 0.43 0.10” |
| designation cross-IDs (Table 1) | M 32 = NGC 221; NGC 205 = M110; NGC 147 = DDO 3; NGC 6822 = IC 4895 / Barnard's Galaxy; IC 1613 = DDO 8 / UGC 668; WLM = DDO 221 / UGCA 444; Leo A = Leo III / DDO 69 / UGC 5364; LGS 3 = Pisces I; Aquarius = DDO 210 ... | verified | p. 90–92, Table 1 | “NGC 6822 IC 4895 L/G dIrr 19h44m56.6s -14d47m21s Barnard (1884) ... DDO 209 Barnard’s Galaxy” |
...
### McConnachie 2018 — The Large-scale Structure of the Halo of the Andromeda Galaxy. II. Hierarchical Structure in the Pan-Andromeda Archaeological Survey
...
| radius of the "classical" regular M31 stellar disc drawn on the PAndAS map | 1.25° (17 kpc) | verified | p. 5, Fig. 4 caption | “a disk of inclination 77◦ and radius 1.25◦ (17kpc)” |

<a id="mcmillan"></a>
### McMillan 2017 — The mass distribution and gravitational potential of the Milky Way
...
- **Copy:** `submittedVersion`
- **Note:** R0 = 8.20 kpc distinguishes it from McMillan 2011 (MNRAS 414, 2446, R0 = 8.29)

| Claim | Value | Status | Page | Passage |
|---|---|---|---|---|
| bulge density form: power law times Gaussian | ρb = ρ0,b / (1 + r′/r0)^α · exp[−(r′/rcut)²] | verified | p. 2, eq. 1 | “The Bissantz & Gerhard model is not axisymmetric, so we make an axisymmetric approximation which has the density profile … with α = 1.8, r0 = 0.075 kpc, rcut = 2.1 kpc, and axis ratio q = 0.5.” |
| Sun–Galactic-Centre distance | R0 = 8.20 ± 0.09 kpc | verified | p. 1, Abstract | “we find that the Sun is R0 = (8.20 ± 0.09) kpc from the Galactic Centre” |
...
<a id="merand2011"></a>
### Mérand 2011 — The nearby eclipsing stellar system δ Velorum III. Self-consistent fundamental parameters and distance
...
<a id="montalto2021"></a>
### Montalto 2021 — The all-sky PLATO input catalogue
...
| Claim | Value | Status | Page | Passage |
|---|---|---|---|---|
| BP−RP → Teff fifth-order polynomial coefficients | 9453.14, −6859.40, 3542.16, −1053.09, 165.635, −10.5672 (in de-reddened colour, (GBP − GRP)0) | verified | p. 11, eq. 4 | “T eff (K) = 9453.14 − 6859.40 (GBP − GRP )0 + + 3542.16 (GBP − GRP )20 + − 1053.09 (GBP − GRP )30” |
...
### Montegriffo 2023 — Gaia Data Release 3: The Galaxy in your preferred colours. Synthetic photometry from Gaia low-resolution spectra
...
### Morrison 2004 — Historical Values of the Earth's Clock Error Delta T and the Calculation of Eclipses
...
<a id="mosenkov2021"></a>
### Mosenkov 2021 — The structure of the Milky Way based on unWISE 3.4 μm integrated photometry

MNRAS 507, 5246-5263 (2021) · [doi:10.1093/mnras/stab2445](https://doi.org/10.1093/mnras/stab2445) · [arXiv:2108.10413](https://arxiv.org/abs/2108.10413)

- **Copy:** `publishedVersion`

| Claim | Value | Status | Page | Passage |
|---|---|---|---|---|
| thick/thin disc luminosity ratio at 3.4 µm | 0.71 ± 0.45 | verified | p. 5254, Table 2 | “LT /Lt 0.71 ± 0.45 0.69 ± 0.20” |
| their thick disc is radially longer than the thin disc |  | verified | p. 5254, Table 2 | “hR, T pc 3219 ± 417” |

<a id="moustakas2023"></a>
### Moustakas 2023 — Siena Galaxy Atlas 2020
...
| Claim | Value | Status | Page | Passage |
|---|---|---|---|---|
| SGA-2020: multi-wavelength optical and infrared imaging atlas of nearby galaxies |  | verified | p. 1, Abstract | “wavelength optical and infrared imaging atlas of 383,620 nearby galaxies” |
...
### Muterspaugh 2010 — The PHASES Differential Astrometry Data Archive. II. Updated Binary Star Orbits and a Long Period Eclipsing Binary
...
| Claim | Value | Status | Page | Passage |
|---|---|---|---|---|
| eta CrB AB (HD 137107) orbit period | P = 15204.9 d | verified | p. 9, Table 6 | “137107 15204.9 42612.9 0.86226 0.27907 58.084 39.885 202.827” |
...
<a id="newburn1973"></a>
### Newburn 1973 — A survey of the outer planets Jupiter, Saturn, Uranus, Neptune, Pluto, and their satellites
...
| Harris's R and I are Hardie's passbands, at effective wavelengths different from Johnson's standards | Hardie R, I (not Johnson 1966) | verified | p. 257, Appendix B | “Harris’ passbands R and I are those of Hardie and are at different effective wavelengths than the standards of Johnson (1966)” |

<a id="nicholson1995"></a>
### Nicholson 1995 — Stellar Occultation Observations of Neptune's Rings: 1984–1988
...
- **Copy:** `publishedVersion` — image-only scan: no text layer, rows read from the page images

| Claim | Value | Status | Page | Passage |
|---|---|---|---|---|
| limits on the Adams and Le Verrier ring optical depths from eight stellar occultations, 1984–1988 |  | verified | p. 295, Abstract | “Data from eight stellar occultations by Neptune between 1984 and 1988 are analyzed to set limits on the optical depths of the continuous Adams and Le Verrier Rings” |

<a id="nimmo2017"></a>
### Nimmo 2017 — Mean radius and shape of Pluto and Charon from New Horizons images

Nimmo F., Umurhan O., Lisse C. M., Bierson C. J., Lauer T. R., Buie M. W., Throop H. B., Kammer J. A., Roberts J. H., McKinnon W. B., Zangari A. M., Moore J. M., Stern S. A., Young L. A., Weaver H. A., Olkin C. B., Ennico K. & the New Horizons GGI team; Icarus 287, 12–29 (2017) · [doi:10.1016/j.icarus.2016.06.027](https://doi.org/10.1016/j.icarus.2016.06.027) · [arXiv:1603.00821](https://arxiv.org/abs/1603.00821)

- **Copy:** `submittedVersion`

| Claim | Value | Status | Page | Passage |
|---|---|---|---|---|
| Pluto mean radius from New Horizons LORRI limb fits | 1188.3 ± 1.6 km (2-σ) | verified | p. 1, Abstract | “The resulting mean radii of Pluto and Charon are 1188.3 ± 1.6 km and 606.0 ± 1.0 km, respectively (2-σ).” |

<a id="north2007"></a>
### North 2007 — γ² Velorum: Orbital Solution and Fundamental Parameter Determination with SUSI
...
### Osterbrock 2006 — Astrophysics of Gaseous Nebulae and Active Galactic Nuclei, 2nd ed.
...
<a id="pace2025"></a>
### Pace 2025 — The Local Volume Database: a library of the observed properties of nearby dwarf galaxies and star clusters
...
| Claim | Value | Status | Page | Passage |
|---|---|---|---|---|
| Local Volume Database: positions, distances, structure and photometry of nearby dwarf galaxies and star clusters |  | verified | p. 20, App. A | “confirmed real: Denotes whether the system should be considered a candidate (=0) or whether the system has been confirmed (=1).” |
| LVDB ra/dec are J2000.0 | J2000.0 | verified | p. 20, App. A | “ra and dec. Location of system [degrees, ICRS frame, J2000.0].” |

<a id="padoan1997"></a>
### Padoan 1997 — The universality of the stellar initial mass function
...
<a id="pecaut2013"></a>
### Pecaut 2013 — Intrinsic Colors, Temperatures, and Bolometric Corrections of Pre-main-sequence Stars
...
- **Copy:** `submittedVersion`
- **Note:** The dwarf sequence is Table 4 and carries no masses; the mass column is in the online "modern mean dwarf" extension, Mamajek 2022 (`mamajek2022`)
...
<a id="pelgrims2020"></a>
### Pelgrims 2020 — Modeling the magnetized Local Bubble from dust data
...
| Claim | Value | Status | Page | Passage |
|---|---|---|---|---|
| Local Bubble inner-surface model: spherical-harmonic reconstruction on a HEALPix grid, variable maximum multipole lmax | lmax 2–10 | verified | p. 1, Abstract; p. 4, Sect. 2.2.2 and Fig. 3 | “We expanded the shell inner surface in spherical harmonics, up to a variable maximum multipole degree” |

<a id="pietrzynski2019"></a>
### Pietrzyński 2019 — A distance to the Large Magellanic Cloud that is precise to one per cent
...
| Claim | Value | Status | Page | Passage |
|---|---|---|---|---|
| LMC distance | 49.59 ± 0.09 (statistical) ± 0.54 (systematic) kpc | verified | p. 1, Abstract; p. 3 | “The final distane is 49.59 ± 0.09 (statistical) ± 0.54 (systematic) kiloparsecs.” |
| LMC distance uncertainty | ± 0.55 kpc (the quadrature sum of the paper's ± 0.09 statistical and ± 0.54 systematic; the paper does not print 0.55) | verified | p. 1, Abstract | “49.59 ± 0.09 (statistical) ± 0.54 (systematic) kiloparsecs” |
...
<a id="piffl2014"></a>
### Piffl 2014 — The RAVE survey: the Galactic escape speed and the mass of the Milky Way
...
| Claim | Value | Status | Page | Passage |
|---|---|---|---|---|
| local Galactic escape speed | 533 +54/−41 km/s (90 % confidence), Galactic rest frame | verified | p. 1, Abstract | “Our best estimate of the local Galactic escape speed ... is 533+54 −41 km s−1 (90% confidence) with an additional 4% systematic uncertainty” |
...
| Claim | Value | Status | Page | Passage |
|---|---|---|---|---|
| flat ΛCDM parameters (Table 7) | H0 = 67.36 ± 0.54 km/s/Mpc, Ωm = 0.3153 ± 0.0073, ΩΛ = 0.6847 ± 0.0073 | verified | p. 19, Table 7 | “H0 . . . 67.36 ± 0.54 ... ΩΛ . . . 0.6847 ± 0.0073 ... Ωm . . . 0.3153 ± 0.0073” |
| SMICA component-separation CMB map |  | verified | p. 10 | “SMICA, which uses an independent component analysis of” |

<a id="porco1995"></a>
### Porco 1995 — Neptune's ring system
...
- **Copy:** `submittedVersion`
- **Note:** The paper states κ = M_B/(M_A+M_B) = 0.4617 ± 0.00044 directly (Table 1, p. 3)

| Claim | Value | Status | Page | Passage |
|---|---|---|---|---|
| α Cen mass ratio κ = M_B/(M_A+M_B) | κ = 0.4617 ± 0.00044 | verified | p. 3, Table 1 | “κ 0.4581 ± 0.00098 0.4617 ± 0.00044” |

<a id="prugniel"></a>
### Prugniel 1997 — The fundamental plane of early-type galaxies: non-homology of the spatial structure?
...
| Sérsic deprojection form ν(u) = u^(−p_n)·exp(−b_n·u^(1/n)) and its own p_n fit | α(n) = 1 − 0.594/n + 0.055/n² | verified | p. 120, App. B.1, eq. B7 | “We determine α by fitting the projected profile with the Sérsic profile. … Then, we fit α(n) by:” |

<a id="ramirez2012"></a>
### Ramírez 2012 — The UBV(RI)C Colors of the Sun
...
### Recio-Blanco 2023 — Gaia Data Release 3: Analysis of RVS spectra using the General Stellar Parametriser from spectroscopy
...
<a id="reid2019"></a>
### Reid 2019 — Trigonometric Parallaxes of High-mass Star-forming Regions: Our View of the Milky Way
...
| Claim | Value | Status | Page | Passage |
|---|---|---|---|---|
| maser-parallax spiral-arm model of the Milky Way |  | verified | p. 2, Abstract | “Fitting log-periodic spirals to the locations of the masers ... allows us to significantly expand our view of the structure of the Milky Way.” |

<a id="rhode2013"></a>
### Rhode 2013 — ALFALFA Discovery of the Nearby Gas-Rich Dwarf Galaxy Leo P. II. Optical Imaging Observations
...
<a id="riello2021"></a>
### Riello 2021 — Gaia Early Data Release 3: Photometric content and validation
...
| Claim | Value | Status | Page | Passage |
|---|---|---|---|---|
| table giving the constellation of a position at equinox 1875.0 |  | verified | p. 695, Abstract | “A table permits rapid determination of the constellation in which an object is located from its 1875.0 position.” |

<a id="ross2020"></a>
### Ross 2020 — The Completed SDSS-IV extended Baryon Oscillation Spectroscopic Survey: Large-scale Structure Catalogs for Cosmological Analysis
...
| Claim | Value | Status | Page | Passage |
|---|---|---|---|---|
| eBOSS DR16 LSS catalogues and target-selection definitions (Sect. 2) |  | verified | p. 3, Sect. 2 | “2 EBOSS TARGETS” |

<a id="samus2017"></a>
### Samus 2017 — General catalogue of variable stars: Version GCVS 5.1
...
<a id="sander2012"></a>
### Sander 2012 — The Galactic WC stars
...
| Claim | Value | Status | Page | Passage |
|---|---|---|---|---|
| WC4 Teff ~117 kK | 117 kK | verified | p. 17, Table 6 | “WC4 117 3310 1.0 −4.65 5.2 10” |
| WC9 Teff ~44 kK | 44 kK (ramp node [9, 44000]) | verified | p. 17, Table 6 | “WC9 44 1390 6.6 −4.80 5.2 10” |
| WC subtype effective temperatures (Table 6) | WC5 83, WC6 78, WC7 71, WC8 60 kK | verified | p. 17, Table 6 | “WC5 83 2780 2.2 −4.62 5.3 12” … “WC8 60 1810 4.2 −4.80 5.3 11” |
...
| Claim | Value | Status | Page | Passage |
|---|---|---|---|---|
| Rigel B double-lined SB orbit: P, T, e, omega | P = 9.860 d, T = JD 2429633.196, e = 0.1, ω = 10° | verified | p. 422, Table 2 | “Period P 9d860 … Periastron passage T JD 2429633.196 G.M.T. … Angle of periastron … 10° … Eccentricity e 0.1” |
| Rigel Ba and Bb both of class B9 | B9 + B9 (no luminosity class) | verified | p. 421 | “eleven of which show satisfactory lines of the components, both of which are of spectral class B9” |

<a id="schaefer2016"></a>
### Schaefer 2016 — Orbits, Distance, and Stellar Masses of the Massive Triple Star sigma Orionis
...
| Claim | Value | Status | Page | Passage |
|---|---|---|---|---|
| sigma Ori distance from orbital parallax 387.5 +/- 1.3 pc | 387.5 ± 1.3 pc | verified | p. 11, Sect. 4.3 | “orbital parallax of π = 2.5806 ± 0.0088 mas gives a distance d = 387.5 ± 1.3 pc to the σ Orionis system” |
...
<a id="schlegel1998"></a>
### Schlegel 1998 — Maps of Dust Infrared Emission for Use in Estimation of Reddening and Cosmic Microwave Background Radiation Foregrounds
...
| Claim | Value | Status | Page | Passage |
|---|---|---|---|---|
| polar reddening: the map's own E(B−V) at the Galactic poles, and the older values it reviews | E(B−V) = 0.015 (NGP) / 0.018 (SGP); older claims E(B−V) ≈ 0.02–0.05 (A(B) ≈ 0.1–0.2), some none | verified | p. 35, Sect. 7.3; p. 34 | “Our estimated reddening, averaged over 10◦ in diameter, is E(B−V ) = 0.015 mag at the NGP and E(B−V ) = 0.018 mag at the SGP … Some authors have claimed that the Galactic poles are essentially free of reddening, whereas others claim a reddening of order E(B−V ) ≈ 0.02 − 0.05 mag, corresponding to an extinction A(B) ≈ 0.1 − 0.2 mag.” |

<a id="simondiaz2011"></a>
### Simón-Díaz 2011 — A third massive star component in the σ Orionis AB system
...
### Simón-Díaz 2015 — Orbital and physical properties of the sigma Ori Aa, Ab, B triple system
...
<a id="sion"></a>
### Sion 1983 — A proposed new white dwarf spectral classification system
...
| Claim | Value | Status | Page | Passage |
|---|---|---|---|---|
| WD temperature index = 50400/Teff, so Teff = 50400/N | Teff = 50400 / N | verified | p. 255, Sect. II b; also p. 253 abstract | “a quantative temperature index from 0 to 9 will be used, defined by 10 times … = 5040/T). This quantity 50,400/T given as an integer, will be a temperature and luminosity indicator” |

<a id="smirnov2002"></a>
### Smirnov 2002 — Optical Properties of Atmospheric Aerosol in Maritime Environments
...
| Claim | Value | Status | Page | Passage |
|---|---|---|---|---|
| maritime aerosol optical depth at 500 nm | mean τa = 0.07, mode 0.06 | verified | p. 501 (abstract); p. 510, Table 3 | “The optical thickness is remarkably stable with mean value of t a (500 nm) 5 0.07, mode value at t am” |

<a id="sneep2005"></a>
### Sneep 2005 — Direct measurement of the Rayleigh scattering cross section in various gases
...
| Pluto: pre-removal Table 2a row plus Table 2b b term | a 39.48686035 ... b -0.01262724 | verified | memo p. 3, Tables 2a/2b (read on the page image; the memo text layer is font-garbled) | “Pluto -0.01262724” |
| Table 2a validity interval | 3000 BC - 3000 AD | verified | approx_pos.html, Table 2a | “valid for the time-interval 3000 … BC -- 3000 … AD” |
...
| Earth–Moon barycentre longitude error | 40" (Table 2a, 3000 BC–3000 AD); 20" (Table 1, 1800–2050) | verified | approx_pos.html, Accuracy | “EM Bary† … 20 8 6 40 15 15” |
...
| Table 1 validity interval (Table 2a spans 3000 BC–3000 AD) | 1800-2050 | verified | approx_pos.html, Table 1 | “valid for the time-interval 1800 … AD - 2050 … AD” |
| EM-barycentre split into Earth + Moon |  | verified | approx_pos.html, Table 2a | “EM Bary = Earth/Moon Barycenter” |
| the series' time argument is TDB | T_eph, equivalent to JD TDB | verified | approx_pos.html, Formulae for using the Keplerian elements | “In order to obtain the coordinates of one of the planets at a given Julian ephemeris date, … (equivalent to the post-2006 IAU Barycentric Dynamical Timescale, JDTDB),” |
| Table 1 (1800–2050) and Table 2a (3000 BC–3000 AD) are separate element sets |  | verified | memo p. 3, Table 1; approx_pos.html, Table 1 | “valid for the time-interval 1800 … AD - 2050 … AD” |

<a id="steinmetz2020"></a>
### Steinmetz 2020 — The Sixth Data Release of the Radial Velocity Experiment (RAVE) – I: Survey Description, Spectra and Radial Velocities

AJ 160, 83 · [arXiv:2002.04377](https://arxiv.org/abs/2002.04377) · [2020AJ....160...83S](https://ui.adsabs.harvard.edu/abs/2020AJ....160...83S)

- **Copy:** `submittedVersion`

| Claim | Value | Status | Page | Passage |
|---|---|---|---|---|
| RAVE DR6: observations and unique stars | 518,387 observations of 451,783 stars | verified | p. 2, abstract | “data release (DR6 or FDR) is based on 518 387 observations of 451 783 unique stars” |

<a id="stelzer2003"></a>
### Stelzer 2003 — Castor A and Castor B resolved in a simultaneous Chandra and XMM-Newton observation
...
| Castor Ab late-K, Bb early-M | late-K, early-M | verified | p. 1, Sect. 1 | “most likely a late-K star ... seems to be an early-M dwarf” |

<a id="sternberg2003"></a>
### Sternberg 2003 — Ionizing Photon Emission Rates from O- and Early B-Type Stars and Clusters
...
| Claim | Value | Status | Page | Passage |
|---|---|---|---|---|
| B0 (class V) ionising photon rate log Q_H | 48.02 | verified | p. 22, Table 1 | “B0 33340 3.932 8.3 4.88 21.2 1853 1.0(-7) 48.02 45.82” |
| log Q_H for luminosity classes III and I (Tables 2–3) | O9: V 48.47, III 48.86, I 49.24; B0: V 48.02, III 48.39 | verified | p. 22-23, Tables 1-3 | “O9 32740 3.333 24.6 5.80 46.7 1847 4.8(-6) 49.24 47.73 … O9 34320 3.621 14.7 5.43 32.6 1952 1.7(-6) 48.86 47.47 … O9 35900 3.959 8.8 5.06 25.4 2083 1.7(-7) 48.47 47.17” |

<a id="strauss2002"></a>
### Strauss 2002 — Spectroscopic Target Selection in the Sloan Digital Sky Survey: The Main Galaxy Sample
...
| Claim | Value | Status | Page | Passage |
|---|---|---|---|---|
| SDSS Main Galaxy Sample selection |  | verified | p. 5, Sect. 1 | “The main galaxy sample consists of galaxies with rP ≤ 17.77 and µ50 ≤ 24.5 magnitudes per square arcsec” |
...
- **Copy:** `ADS scan of published article` — PDF page n is printed page 525 + n

| Claim | Value | Status | Page | Passage |
|---|---|---|---|---|
| Strömgren radius (ionised-sphere radius) R_S |  | verified | p. 530, eq. 16 | “The value of s corresponding to … = 1, which we shall call s0, is, according to (10),” |

<a id="subramanian2012"></a>
### Subramanian 2012 — THE THREE-DIMENSIONAL STRUCTURE OF THE SMALL MAGELLANIC CLOUD
...
<a id="tempel2011"></a>
### Tempel 2011 — SDSS surface photometry of M 31 with absorption corrections
...
| M31 intrinsic total m_V | 3.24 | verified | p. 1, Abstract | “the total intrinsic absorption-corrected luminosities of M 31 in the B and the V filters are 4.10 and 3.24 mag, respectively.” |
...
<a id="tomasko2008"></a>
### Tomasko 2008a — A model of Titan's aerosols based on measurements made inside the atmosphere
...
- **Copy:** `publishedVersion`
- **Note:** The Titan ground-light fraction is in the companion heat-balance paper, Tomasko et al. 2008b (`tomasko2008b`) (PSS 56, 648)

| Claim | Value | Status | Page | Passage |
|---|---|---|---|---|
| Titan haze extinction optical depth vs wavelength (Table 3 fits, summed) | τ ≈ 11.7 at 450 nm, 8.1 at 550, 6.0 at 650 (fits 1.012×10⁷λ^−2.3339 + 2.029×10⁴λ^−1.409 + 6.270×10²λ^−0.971, λ in nm, evaluated) | verified | p. 699, Fig. 52; p. 697, Table 3 and Fig. 50 | “The vertical distribution of cumulative extinction optical depth at 500 nm is shown versus altitude.” |

<a id="tomasko2008b"></a>
### Tomasko 2008b — Heat balance in Titan's atmosphere
...
<a id="torra2021"></a>
### Torra 2021 — Gaia Early Data Release 3: Building the Gaia DR3 source list — Cross-match of Gaia observations
...
<a id="torres2002"></a>
### Torres 2002 — Absolute Dimensions of the M-Type Eclipsing Binary YY Geminorum (Castor C)
...
| YY Gem component masses | formal 0.5975 ± 0.0047 and 0.6009 ± 0.0047 M☉; adopted mean 0.5992 ± 0.0047 M☉ each | verified | p. 9, Sect. 5; p. 33, Table 5 (adopted mean, 0.5992 ± 0.0047 each) | “the absolute masses we derive for the two stars are formally different (MA = 0.5975 ± 0.0047 M⊙ and MB = 0.6009 ± 0.0047 M⊙)” |
| YY Gem spectral type as the paper quotes it | M1.0 Ve | verified | p. 1, Sect. 1; p. 3 | “YY Gem (M1.0 Ve, mass ∼ 0.6 M⊙ ; Bopp 1974; Leung & Schneider 1978)” |
...
<a id="torres2022"></a>
### Torres 2022 — The Orbits and Dynamical Masses of the Castor System
...
| Claim | Value | Status | Page | Passage |
|---|---|---|---|---|
| Castor AB (STF1110 AB) visual orbit period | P = 459.1 yr | verified | p. 8, Table 5 | “PAB (year) 459.1 ± 2.3” |
| Castor AB semimajor axis | a = 6.722″ | verified | p. 9, Table 5 (cont.) | “a″ … AB (″) 6.722 ± 0.021” |

<a id="tully2023"></a>
### Tully 2023 — Cosmicflows-4
...
| Claim | Value | Status | Page | Passage |
|---|---|---|---|---|
| Cosmicflows-4: 55,877 galaxies in 38,065 groups with distances | 55,877 galaxies in 38,065 groups | verified | p. 1, abstract | “With Cosmicflows-4, distances are compiled for 55,877 galaxies gathered into 38,065 groups.” |

<a id="vaidman2025"></a>
### Vaidman 2025 — Evaluating Gaia Astrometric Quality and Distances for Galactic Hot Supergiants
...
### Vallenari 2023 — Gaia Data Release 3
...
### van der Kruit 1986 — Surface photometry of edge-on spiral galaxies. V. The distribution of luminosity in the disk of the Galaxy derived from the Pioneer 10 background experiment
...
| Claim | Value | Status | Page | Passage |
|---|---|---|---|---|
| LMC disc scale length | 1.5 kpc | verified | p. 22, Sect. 8; p. 12, Sect. 4.3 | “The best-fitting exponential disk scale length is now rd ≈ 1.5 kpc” |

<a id="vandermarel2014"></a>
### van der Marel 2014 — THIRD-EPOCH MAGELLANIC CLOUD PROPER MOTIONS. II. THE LARGE MAGELLANIC CLOUD ROTATION FIELD IN THREE DIMENSIONS
...
| LMC disc inclination and line-of-nodes PA from the PM velocity field | i = 39.6° ± 4.5°, Θ = 147.4° ± 10.0° | verified | p. 6, Table 1; p. 14, Sect. 4.2 | “Our best-fit model to the PM velocity field has i = 39.6◦ ± 4.5◦ and Θ = 147.4◦ ± 10.0◦” |
...
| Claim | Value | Status | Page | Passage |
|---|---|---|---|---|
| new reduction of the Hipparcos astrometric data (HIP2) |  | verified | p. 1, Abstract; p. 5 | “A new reduction of the astrometric data as produced by the Hipparcos mission has been published” |
...
| Claim | Value | Status | Page | Passage |
|---|---|---|---|---|
| log-normal volume-density PDF of compressible, transonic flow (2D simulation, rms Mach number 0.58, local maximum 1.85) |  | verified | p. 682, Sect. 1; p. 686, Sects. 4.2–4.3; p. 687, Fig. 3 | “shows that the density pdf is, at least in the transonic regime, very well approximated by a lognormal distribution” |

<a id="vondrak2011"></a>
### Vondrák 2011 — New precession expressions, valid for long time intervals
...
- **Copy:** `publishedVersion`
- **Note:** Corrigendum A&A 541, C1 (2012), not held (A&A returns 403): it corrects Q_A term C7 at P = 882 cy from 198.296071 (as printed in the copy) to 198.296701
...
| ecliptic-pole series P_A, Q_A: cubic polynomial + 8 periodic terms (Table 1, Eq. 8) | 8 periodic terms, P = 708.15 … 547.00 cy; Q_A C7 at 882 cy printed as 198.296071 | verified | p. 4, Table 1 and Eq. (8) | “Table 1. Periodic terms in PA , QA .” … “C7 –87.676083 198.296071 882.00” |
| equator-pole series X_A, Y_A: cubic polynomial + 14 periodic terms | cubic polynomial + 14 periodic terms; X_A = 5453.282155 + 0.4252841T + … | verified | p. 5, Table 2 and Eq. (9); pp. 14–15, ltp_PEQU listing | “XA = 5453.282155 + 0.4252841T” |
...
### Wainscoat 1992 — A model of the 8-25 micron point source infrared sky
...
| Claim | Value | Status | Page | Passage |
|---|---|---|---|---|
| the SKY model also operates at JHK and visible BV wavelengths |  | verified | p. 112, Sect. 1 | “our model also operates at JHK and BV(visible light) wavelengths, permitting self-consistency checks” |

<a id="weingartner2001"></a>
### Weingartner 2001 — Dust Grain-Size Distributions and Extinction in the Milky Way, Large Magellanic Cloud, and Small Magellanic Cloud
...
| Claim | Value | Status | Page | Passage |
|---|---|---|---|---|
| dust models fit for three R_V cases; R_V above 3.1 is observed for dense clouds | R_V = 3.1, 4.0, 5.5 (model cases) | verified | p. 2, Sect. 1; p. 7, Sect. 3.1 | “For the diffuse ISM, RV ≈ 3.1; higher values are observed for dense clouds. … In Table 1 we list the values of the distribution parameters for which the extinction with RV = 3.1, 4.0, and 5.5 is best fit” |
| grain growth as the cause |  | verified | p. 13, Sect. 4.3 | “small grains coagulate onto large grains in relatively dense environments, as expected” |

<a id="wenger2000"></a>
### Wenger 2000 — The SIMBAD astronomical database
...
| Claim | Value | Status | Page | Passage |
|---|---|---|---|---|
| solar absolute V magnitude | 4.81 (Vega) | verified | p. 1, Abstract; p. 6, Table 3 (Johnson V) | “estimated absolute magnitudes of the Sun are MB = 5.44, MV = 4.81 and MK = 3.27 mag in the vegamag system” |

<a id="wilquet2009"></a>
### Wilquet 2009 — Preliminary characterization of the upper haze by SPICAV/SOIR solar occultation in UV to mid-IR onboard Venus Express
...
<a id="wilson1953"></a>
### Wilson 1953 — General Catalogue of Stellar Radial Velocities

Carnegie Institution of Washington Publ. 601, Washington D.C. (1953) · [1953GCRV..C......0W](https://ui.adsabs.harvard.edu/abs/1953GCRV..C......0W)

- **Copy:** `VizieR ReadMe`

| Claim | Value | Status | Page | Passage |
|---|---|---|---|---|
| GCRV catalogue bibcode | 1953GCRV..C......0W | verified | l. 6 | “=1953GCRV..C......0W” |
...
### Wittkowski 2016 — Near-infrared spectro-interferometry of Mira variables and comparisons to 1D dynamic model atmospheres and 3D convection simulations
...
<a id="wolff2009"></a>
### Wolff 2009 — Wavelength dependence of dust aerosol single scattering albedo as observed by the Compact Reconnaissance Imaging Spectrometer
...
| Claim | Value | Status | Page | Passage |
|---|---|---|---|---|
| Mars dust single-scattering albedo, red channel | ω̃ ≈ 0.97 | verified | p. 11, Fig. 12 | “Figure 12. Average … spectra (both MER sites) as a … function of the assumed reff.” |
| Mars dust single-scattering albedo, green channel | ω̃ ≈ 0.90 | verified | p. 11, Fig. 12 | “Figure 12. Average … spectra (both MER sites) as a … function of the assumed reff.” |
| Mars dust single-scattering albedo at the bluest measured point | ω̃ ≈ 0.77 at 440 nm | verified | p. 11, Fig. 12 | “Figure 12. Average … spectra (both MER sites) as a … function of the assumed reff.” |

<a id="woodruff2008"></a>
### Woodruff 2008 — The Keck Aperture Masking Experiment: Multiwavelength Observations of Six Mira Variables
...
<a id="woodruff2009"></a>
### Woodruff 2009 — The Keck Aperture Masking Experiment: Spectro-interferometry of Three Mira Variables from 1.1 to 3.8 um
...
<a id="wyman2013"></a>
### Wyman 2013 — Simple Analytic Approximations to the CIE XYZ Color Matching Functions
...
| Claim | Value | Status | Page | Passage |
|---|---|---|---|---|
| multi-lobe piecewise-Gaussian CIE 1931 CMF fit coefficients | x̄: 0.362/442.0/16.0/26.7, 1.056/599.8/37.9/31.0, −0.065/501.1/20.4/26.2; ȳ: 0.821/568.8/46.9/40.5, 0.286/530.9/16.3/31.1; z̄: 1.217/437.0/11.8/36.0, 0.681/459.0/26.0/13.8 (amplitude/centre/σ_lo/σ_hi, nm; σ = 1/γ, 1/δ, Table 1 printing the inverse widths γ, δ) | verified | p. 4, Table 1 and Eq. (4) | “α 0.362 1.056 -0.065 0.821 0.286 1.217 0.681” |
| fits reproduce the tabulated CMFs to ~1% | ~1% (derived: the paper gives squared errors, not a percentage — max 2.0e-4 / 6.4e-5 / 4.9e-4, mean 3.1e-5 / 7.1e-6 / 1.6e-5 for x̄ / ȳ / z̄, i.e. ~0.3–2 % in CMF units) | verified | p. 6, Table 2 | “1931 Multi-lobe fit (Sec. 2.2) 2.0e-4 6.4e-5 4.9e-4 3.1e-5 7.1e-6 1.6e-5 … Table 2. Maximum and mean squared error for fits of the CIE standard observers” |

<a id="zacharias2012"></a>
### Zacharias 2012 — UCAC4 (VizieR I/322A)

Zacharias N., Finch C. T., Girard T. M., Henden A., Bartlett J. L., Monet D. G. & Zacharias M. I.; AJ 145, 44 (2013) · [2013AJ....145...44Z](https://ui.adsabs.harvard.edu/abs/2013AJ....145...44Z) · [2012yCat.1322....0Z](https://ui.adsabs.harvard.edu/abs/2012yCat.1322....0Z)

- **Copy:** `VizieR ReadMe`

| Claim | Value | Status | Page | Passage |
|---|---|---|---|---|
| UCAC4 catalogue bibcode | 2012yCat.1322....0Z | verified | l. 7 | “=2012yCat.1322....0Z” |

<a id="zhang2023"></a>
### Zhang 2023 — Parameters of 220 million stars from Gaia BP/RP spectra
...
| Claim | Value | Status | Page | Passage |
|---|---|---|---|---|
| E_ZGR is the ZGR23 extinction unit: a scalar amount of extinction along the line of sight |  | verified | p. 5, Sect. 2.5 model (Eq. 7) | “where 𝐸 is a scalar measurement of the amount of extinction in front of the star along the line of sight” |
...
| Claim | Value | Status | Page | Passage |
|---|---|---|---|---|
| molecular-cloud distances (Table A.1 sightlines) | 326 sightlines | verified | p. 4, Sect. 3 | “In Table A.1, we summarize our distance results for the 326 sightlines targeted towards clouds in the Star Formation Handbook.” |
...
| Claim | Value | Status | Page | Passage |
|---|---|---|---|---|
| 3D cloud extents (Table 1), 12 clouds incl. Corona Australis | per cloud: l, b, d min/med/max, x/y/z min/max | verified | p. 8, Table 1 | “Chamaeleon 299.0 -15.9 173 183 190 69 99 -159 -148 -57 -35 1 68” |
...
| Plummer profile fits n0, R_flat, p (Table 2), 11 clouds | per cloud: n0, R_flat, p | verified | p. 7, Eq. (4); p. 11, Table 2 | “our free parameters are n0 (peak profile height), Rflat (the flattening radius), and p (index of the density profile)” |
...
| cloud masses from the NICEST column (Table 3) | e.g. Taurus 1.6×10⁴ M☉ | verified | p. 18, Table 3 | “Taurus 1.6e+04 1.5e+04 1.0 0.90 0.38” |
| peak A_K at Leike resolution | 0.19–0.38 | verified | p. 18, Table 3 | “Musca 5.9e+02 4.9e+02 1.2 0.47 0.19” |
| peak A_K NICEST for Taurus / Ophiuchus | 0.90 / 1.99 | verified | p. 18, Table 3 | “Ophiuchus 9.4e+03 7.9e+03 1.2 1.99 0.31” |
| NICEST-to-3D-dust mass ratio | 1.0–1.6× for clouds wholly inside the Leike grid, up to ~14× for clouds at its edge | verified | p. 19, Sect. 5 | “the ratio of the NICEST mass to the 3D dust mass ranges from 1.0 to 1.6, with an average of 1.2, for the “complete clouds,” |

<a id="zucker2022"></a>
### Zucker 2022 — Star formation near the Sun is driven by expansion of the Local Bubble
