...
Pipeline-derived output plus one hand-curated input. Lives alongside
its source folders under `data/` so the binary-system pipeline's data
stays in one place.

```
multiples.tsv                   build-binaries.py output. Two rows per
                                kept physical WDS pair, plus standalone
                                rows for SIMBAD-known components the
                                pair walk didn't reach. ~5.5 MB, LFS.
component_sptype_overrides.tsv  Hand-curated per-component MK types for
                                ...
                                Castor Ab/Bb). Top tier of Stage 6's
                                spectral cascade (spect_via=curated).
...
`orbit_via`, `spect_via`, `photometry_via`) name which strategy /
catalogue tier supplied each piece of data. Canonical values live in
