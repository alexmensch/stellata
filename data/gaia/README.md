...
- **Source tables** (queried via ADQL through astroquery):
  ...
  - `gaia_dr3_astrometry.tsv` ← `gaia_source` (subset queried by
    deduped source_id).
...
`npm run refresh:gaia-{hip,tyc,astrometry,nss,apsis}` →
[`scripts/refresh/`](../../scripts/refresh/README.md). DR4 transition
order is documented there. The `astrometry` pull reads
`gaia_astrometry_source_id_request.tsv` as input, so it must run
**after** a fresh `npm run build:binaries`.
