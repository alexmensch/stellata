...
const float GLOW_COLUMN_TAU = 1.5;
...
void main() {
  ...
  // Ray through the unit-sphere envelope (analytic, so BackSide
  ...
  // Whisper glow: the shared fresnel-rim shape at the ray's entry point
  // (the Local Bubble treatment), textured by the fine octaves and
  // faded by the column so it never reads off-cloud. Suppressed with
  // the camera inside — the fresnel-shell hide-when-inside contract,
  // for the glow only; the absorption alpha above keeps working from
  // inside the cloud.
  float insideGate = smoothstep(1.0, 1.15, length(ro));
  vec3 glow = vec3(0.0);
  if (insideGate > 0.0) {
    ...
    float presence = 1.0 - exp(-GLOW_COLUMN_TAU * av);
