...
- `shards/` — the population-shard contract (`StarShardTable` flat
  ...
  NOT shard-aware yet](shards/README.md#what-is-not-shard-aware-yet) lists the legs still indexing `catalog` directly.
...
- `star-color-routing-pure.ts` (+ test) — `bestApsisTeff`: picks
  ...
  See [Colour routing](#colour-routing).
...
rendering](perceptual-disc/README.md#physical-size-rendering)), the plate-scale derivation of K that makes star size
invariant in FOV and viewport ([Angular-size calibration](perceptual-disc/README.md#angular-size-calibration)), and the
super-Gaussian kernel with the disc pass's halo-transparency and
discard-fringe rules ([Star intensity profile](perceptual-disc/README.md#star-intensity-profile)). The debug-panel knobs
