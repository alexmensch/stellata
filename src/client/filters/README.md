...
- `filter-controller.ts` — `FilterController`. Owns the single live
  ...
  drive `SceneDeclutter`
  ([Detail-level declutter cycle](../scene/declutter/README.md#detail-level-declutter-cycle)).
...
question. [§ 3.4](/docs/science-hdr-pipeline.md#34-the-instrument-record--aperture-is-the-single-number) carries the record shape
...
([Extended sources](../hdr/emission/README.md#extended-sources--two-solid-angles-one-write-tail)). Its only consumer is
...
[§ 3.3](/docs/science-hdr-pipeline.md#33-fov-is-magnification-the-instrument-is-aperture) for why FOV must not buy depth.
...
([Physical-luminance emission](../star-pipeline/README.md#physical-luminance-emission)), none of them
...
tone-map's `DR_MAG`, which is the real lever ([Operator](../hdr/tonemap/README.md#operator)).
...
([The unit](../hdr/attachments/README.md#the-unit)), so moving K cannot move the
...
luminance unit ([Emission layer](../local-group/README.md#emission-layer)); nothing
...
- **Events.** Every mutation emits `'filter'` then `'state'` (the
  emission-pairing contract in [Event bus](/src/client/README.md#event-bus-on-stellata)).
