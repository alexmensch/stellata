...
/**
 * Render a major × minor axis pair as "<major> × <minor> <unit>" — one
 * unit suffix, at the end. Local Group semi-axes (~50 pc — ~30 kpc) and
 * cloud semi-axes (~5 pc — ~90 pc) both land in the same `fmtDist`
 * decade tier in practice, so the single suffix reads consistently.
 */
