...
- **`recenter` rebases every `localPc`, not just Sol's.** The hard-focus
  setter reads `localPositionInto` immediately after the recentre it
  triggered; samples left in the pre-recentre frame would shift the
  camera by the whole recentre delta. This is what `hostLocalPos` does
  for the planet field.
...
The trail's mirror stroke takes the seam's `localPass` flag — the
repo-wide requirement for anything rendering in both passes. The marker
needs no mirror variant at all: reversed-z deleted the log-depth stage
that was the two variants' only difference
