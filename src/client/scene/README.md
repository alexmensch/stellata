...
LG-emission `setEnabled`, orbit rings, heliopause shell, Local Bubble
shell) have no per-frame gate, so their bind *pushes* the change
imperatively. A per-element
