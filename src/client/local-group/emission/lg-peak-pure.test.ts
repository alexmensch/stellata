...
describe('the bound against the shader', () => {
  it('contains the mirror-phase central ray for every object, camera and plate scale', () => {
    ...
    // Equality where the midpoint phase is the maximum; 1e-9 is rounding.
  ...
  it('the phase maximum exceeds the midpoint phase by 0.82 mag on the M31 bulge from 8 mesh radii', () => {
    ...
    expect(-2.5 * Math.log10(peak / midpoint)).toBeCloseTo(-0.82, 2);
  });
});
