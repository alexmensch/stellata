...
slots on top of the fresh baselines in the same frame. Unfocused, the
walk does NOT read the float32 `catalog.positions` for its slot-reset
baseline — at a star's distance the float32 absolute ULP (~0.4 AU at
28 pc) is coarser than a tight orbit, so subtracting the origin from the
rounded absolute snaps the whole system onto that grid frame to frame
under scrub. It reconstructs the local baseline in float64 instead —
`(base + v·Δt) − worldOffset` via `writeAdvancedLocal`, off the immutable
J2016.0 baseline + velocities (see § Walk-active LOD). When a star is
focused every relation resets from `catalog.positions` so the whole
scene rides the same absolute the shell's epoch-follow moves the camera
by (`focalPerturbationInto`'s `bakedDiff` also reads the live
`catalog.positions`) — the two cancel and the focal stays pinned. The
systemic-velocity invariant keeps every `has_orbit` pair's baked diff
constant under the advance, so no cache here is epoch-keyed. (The one deliberate exception: `baseDiffPc`'s ICRS
