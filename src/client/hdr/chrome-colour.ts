...
export function setBuiltinChromeColour(target: THREE.Color, hex: number): THREE.Color {
  return assign(target, decode(srgbFromHex(hex)));
}
...
export function setRawChromeColour(target: THREE.Color, hex: number): THREE.Color {
  return assign(target, decode(decode(srgbFromHex(hex))));
}
