...
arrives as a real 404 (`not_found_handling = "none"` in `wrangler.toml`),
which the `!res.ok` branch answers; the parse-error branch below it still
covers an artifact that is present but truncated.
