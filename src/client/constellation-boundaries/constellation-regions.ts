...
import { constellationKey, createGridConstellationLookup } from './iau-boundaries-pure';
...
/** The IAU-88 table keyed the way the region codes resolve — lowercase
 *  3-letter code, which is what `constellationKey` produces. */
function indexByKey(
  constellations: readonly ConstellationTableEntry[],
): Map<string, number> {
  return new Map(constellations.map((c, i) => [c.code.toLowerCase(), i]));
}
 ...
 *  the current epoch, a galaxy, a cloud. The grid comes off the artifact the
 *  build emitted from the same decomposition that assigned byte 34, so a star
 *  and the planet beside it are answered by one partition, not two. */
export function createConstellationNamer(
  artifact: BoundaryArtifact,
  constellations: readonly ConstellationTableEntry[],
): ConstellationNamer {
  const lookup = createGridConstellationLookup(decodeRegionGrid(artifact.regions));
  const indices = indexByKey(constellations);
  return {
    nameAt(absolutePc) {
...
/** The label anchors baked to absolute ICRS at `SPHERE_RADIUS_PC`, exactly as
...
export function buildConstellationLabelAnchors(
  artifact: BoundaryArtifact,
  constellations: readonly ConstellationTableEntry[],
): ConstellationLabelAnchor[] {
  const indices = indexByKey(constellations);
  const anchors: ConstellationLabelAnchor[] = [];
