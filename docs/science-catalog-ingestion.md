...
Covers the AT-HYG/Gaia/Hipparcos merge, the [Bailer-Jones 2021](/data/papers/index.md#bailerjones2021) and
...
- **Still current.** The distance stack ([Bailer-Jones 2021](/data/papers/index.md#bailerjones2021) → LMC kinematic →
...
just past the LMC distance (49.59 kpc, [Pietrzyński 2019](/data/papers/index.md#pietrzynski2019)) because the
...
2. **LMC kinematic override** — replaces B-J's mis-anchored posterior
   ...
   identification pinned to [Pietrzyński 2019](/data/papers/index.md#pietrzynski2019)'s eclipsing-
...
Ordering is non-commutative. [Bailer-Jones 2021](/data/papers/index.md#bailerjones2021) runs first because its
...
**[Bailer-Jones 2021](/data/papers/index.md#bailerjones2021) DR3 distance override (Layer 1).** AT-HYG's `dist` for
...
2–5 kpc. [Bailer-Jones 2021](/data/papers/index.md#bailerjones2021) (CDS I/352)
...
[Lindegren 2021](/data/papers/index.md#lindegren2021) is applied upstream in B-J's own pipeline, so we consume the posteriors directly without
...
rows we take the [Bailer-Jones 2021](/data/papers/index.md#bailerjones2021) distance (photogeometric `r_med_photogeo`
...
[Bailer-Jones 2021](/data/papers/index.md#bailerjones2021) publication and keep their AT-HYG values. The override
...
[Bailer-Jones 2021](/data/papers/index.md#bailerjones2021) input. The harness is built to re-run on every distance-
source change — a DR4 [Bailer-Jones 2021](/data/papers/index.md#bailerjones2021) refresh, a switch to StarHorse or a
...
**LMC kinematic distance refinement (Layer 2).** [Bailer-Jones 2021](/data/papers/index.md#bailerjones2021)'s
...
[van der Marel 2014](/data/papers/index.md#vandermarel2014)) whose proper motion lies within
...
(49.59 kpc, [Pietrzyński 2019](/data/papers/index.md#pietrzynski2019), shipped as 49.594; CDS
...
([Creevey 2023](/data/papers/index.md#creevey2023)). It publishes two independent
solutions per source: `gspphot` (photometric fit to BP/RP spectra +
parallax — [Andrae 2023](/data/papers/index.md#andrae2023)) and `gspspec`
(spectroscopic fit to RVS spectra —
[Recio-Blanco 2023](/data/papers/index.md#recioblanco2023)). Each emits (T_eff, log g, [M/H]);
...
- **Stellar colour calibration** uses Apsis T_eff as the intrinsic
  ...
  beats [Ballesteros 2012](/data/papers/index.md#ballesteros2012)(B-V) here (gspphot fits include `A0` explicitly,
...
from the [van Leeuwen 2007](/data/papers/index.md#vanleeuwen2007) HIP2 file we already commit, to the printed 4 dp
...
The [Bailer-Jones 2021](/data/papers/index.md#bailerjones2021) override already recomputes xyz from printed ra/dec
...
2. **HIP2 [van Leeuwen 2007](/data/papers/index.md#vanleeuwen2007)** (ra, dec at the Hipparcos catalogue epoch J1991.25 — [ESA 1997](/data/papers/index.md#esa1997), p. xv — PM-propagated forward to
...
first-hand: an ordered cascade over Gaia DR3, HIP2, CNS5, [Gliese 1991](/data/papers/index.md#gliese1991)
V/70A and bibcoded SIMBAD values settles which parallax a record
inverts, and the [Bailer-Jones 2021](/data/papers/index.md#bailerjones2021) → LMC-kinematic → cutoff stack above
...
2. **HIP2 PM** for the rows the cascade routes to HIP2 (2,509
   ...
   `data/hipparcos/` [van Leeuwen 2007](/data/papers/index.md#vanleeuwen2007) columns already committed.
...
[Butkevich 2014](/data/papers/index.md#butkevich2014) add, which exceed 0.1 mas over 100 yr for
...
- **Load-epoch caches stay put.** The Sol-distance machinery
  ...
  drift is sub-voxel for the [Edenhofer 2024](/data/papers/index.md#edenhofer2024) grid.

**Scrub-range clamp — what moves with `t`, and why nothing fades.**
The model clock is clamped to the [Standish 1992](/data/papers/index.md#standish1992) ephemeris window
...
planets are arcsecond-faithful inside 1900–2100 and within [Standish 1992](/data/papers/index.md#standish1992)'s
...
array and deform with the stars; GCVS pulsation phase and binary
orbits already run on `getT()`. The layers with no kinematics in
their source data — molecular clouds, dust, Milky Way volume,
galactic grid, Local Group wireframes, heliopause — are *genuinely
static* on a ±3 kyr scale (Milky Way rotation shifts kpc-scale
structure by < 1 pc; cloud morphology evolves on 10⁵–10⁶ yr), so
rendering them fixed is physically correct, not a frozen lie. The
misleading-freeze problem this epic once contemplated only exists at
≥100 kyr offsets, which the clamp — forced anyway by the ephemeris
and linear-PM validity — never exposes. Known limitation: the ~950
Tier-3 wide pairs with divergent baked velocities shear slowly under
deep scrub, up to tens of arcminutes at the window edges for the
worst pairs.
