...
(`../src/client/webgpu/tonemap-tsl.ts`, exported alongside a test-pinned pure
TS mirror) consumed by both the fullscreen pass and the no-float-RT
fallback (§ 6) — the `../src/client/webgpu/extinction/dust-raymarch-tsl.ts` two-consumers pattern.
