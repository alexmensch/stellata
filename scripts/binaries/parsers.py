...
# exceeds the 5-char field width, WDS writes exactly 999.9 rather than
...
# the secondary pass falls through to per-component identifier bindings
# (SIMBAD, CCDM) once this collapses to None.
WDS_RHO_OVERFLOW_SENTINEL_ARCSEC = 999.9
...
def parse_wds_sep_pa(s: str) -> float | None:
    ...
    orbital-element level — Spica) and exactly 999.9 when the value
    overflows the field width (ultra-wide pairs whose (ρ, θ) can't
    localise the secondary). ρ and θ are physically non-negative, so a
    negative value is the no-measurement sentinel and 999.9 the overflow
    marker; collapsing both here stops either from leaking past this
    ...
    v = safe_float(s)
    if v is None or v < 0.0 or v >= WDS_RHO_OVERFLOW_SENTINEL_ARCSEC:
        return None
    return v
