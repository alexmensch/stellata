...
describe('scenarios', () => {
  ...
  it('composes --hash with the backend fragment: both present, either alone, neither', () => {
    expect(scenarioUrl('http://h', 'B', 'webgl2', 'webgpu-gate=force')).toBe('http://h/app/v/B/#renderer=webgl2&webgpu-gate=force');
    expect(scenarioUrl('http://h', 'B', 'webgpu', 'webgpu-gate=force')).toBe('http://h/app/v/B/#webgpu-gate=force');
    expect(scenarioUrl('http://h', 'B', 'webgl2', '')).toBe('http://h/app/v/B/#renderer=webgl2');
    expect(scenarioUrl('http://h', 'B', 'webgpu', '')).toBe('http://h/app/v/B/');
  });
});
