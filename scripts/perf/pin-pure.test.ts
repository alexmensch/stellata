...
describe('pinProvenanceLines — what the pin measured, before any row is read', () => {
  ...
  it('says nothing when the commit landed and main has not moved under src/client', () => {
    expect(pinProvenanceLines(onMain, 'landed', { files: 0, insertions: 0, deletions: 0 })).toEqual([]);
  });

  it('names a pre-squash tip that no hash on main carries', () => {
    const lines = pinProvenanceLines(onMain, 'unlanded', { files: 0, insertions: 0, deletions: 0 });
    expect(lines[0]).toContain('not an ancestor of origin/main');
    expect(lines[0]).toContain('pre-squash branch tip');
  });

  it('quotes the render-path drift a mark might really be', () => {
    const lines = pinProvenanceLines(onMain, 'unlanded', { files: 42, insertions: 1600, deletions: 30 });
    expect(lines[1]).toContain('42 files, +1600/-30');
    expect(lines[1]).toContain('may be that drift rather than this diff');
  });

  it('says so when the pin records no main base at all', () => {
    const based = { ...pin, git: { ...pin.git, mainCommit: null } };
    expect(pinProvenanceLines(based, 'unlanded', null).at(-1)).toContain('cannot be measured at all');
  });

  it('separates an unreadable ancestry from a known-unlanded one', () => {
    expect(pinProvenanceLines(onMain, 'unknown', null)[0]).toContain('drift is unbounded');
  });
});
