...
class MembershipReaderTests(unittest.TestCase):
    ...
    def test_iter_membership_rows_yields_named_cells(self) -> None:
        rows = list(rl.iter_membership_rows(self._table("1-2-1\t2\t99\tHYG\n")))
...
class DeepPopulationAdqlTests(unittest.TestCase):
    def test_magnitude_leg_qualifies_the_bound_with_the_joined_alias(self) -> None:
        adql = rl.magnitude_leg_adql(["source_id", "x"], "cat.t", ("1.0", "2.0"))
        self.assertIn(f"JOIN {rl.GAIA_SOURCE_TABLE} AS g ON g.source_id = t.source_id",
                      adql)
        self.assertIn(f"WHERE g.{rl.G_MAG_COLUMN} > 1.0", adql)

    def test_magnitude_leg_projects_unrenamed_columns(self) -> None:
        # Both legs feed one collector, so their column names must agree.
        adql = rl.magnitude_leg_adql(["source_id", "x"], "cat.t", (None, "2.0"))
    ...
    def test_request_leg_is_an_unjoined_in_clause(self) -> None:
        adql = rl.request_leg_adql(["source_id", "x"], "cat.t", [7, 9])
        self.assertEqual(
