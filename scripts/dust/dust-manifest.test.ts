...
// chunk-integrity test self-skips on pointer stubs (real in CI's `Tier-A star corpus` step).
...
describe('data/dust/manifest.json encode contract', () => {
  ...
  it('pins the per-cloud peak-column check against the Zucker targets', () => {
    ...
    // (/docs/science-molecular-clouds.md#4-per-cloud-density-model--the-presence-pass-field). A collapse below these levels means
