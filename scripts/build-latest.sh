#!/usr/bin/env bash
# `pnpm run build`, first stopping any build:latest already running in this checkout.
set -euo pipefail
...
if [ -f "$lock" ]; then
  ...
  if [[ "$old" =~ ^[0-9]+$ ]] && ps -o command= -p "$old" 2>/dev/null | grep -q 'run build'; then
    echo "build:latest: stopping the running build (process group $old)"
...
if [ "$(cat "$lock" 2>/dev/null)" != "$pgid" ]; then
  echo "build:latest: superseded by a newer build:latest"
  exit 0
fi
rm -f "$lock"
exit "$status"
