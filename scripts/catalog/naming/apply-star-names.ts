 ...
 *  authority cannot express, never as a home for the folk names /docs/star-naming.md#2-authority--one-source-approves-names-everything-else-compiles-them routes
 *  to aliases. */
...
export interface StarNamingCounts {
  ...
  /** Records carrying a /docs/star-naming.md#2-authority--one-source-approves-names-everything-else-compiles-them string designation (`Ross 128`). */
  namingEponym: number;
...
export function applyStarNames(
  ...
  for (const star of stars) {
    ...
    if (name !== null) {
      ...
      // from the /docs/star-naming.md#2-authority--one-source-approves-names-everything-else-compiles-them disposition gate rather than absent from it.
