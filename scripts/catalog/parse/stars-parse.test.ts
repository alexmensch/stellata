...
import { avSolToStar, R_V, type DustGrid } from '../distance/dust/dust-deextinction-pure';
import {
  CATALOG_SCENE_EPOCH,
  directionAtEpoch,
...
function glieseParallaxes(
  ...
  return {
    byKey: new Map(rows.map((r) => [r.gl, {
      name: `Gl ${r.gl}`, comp: '',
      vMag: null, bMinusV: null, spectral: null, hd: null,
  ...
  it('takes the manifest column even where the G−V gate would scrub it', () => {
    ...
    const { stars } = readStars(
      ...
      }]),
      {
        ...
        // since this Gaia row carries no BP/RP for the Riello 2021
