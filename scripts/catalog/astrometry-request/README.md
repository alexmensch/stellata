...
rows and the union total re-derive together in one local run. Correct them
from that run and never one cell from another source: the increments are what
make the four sum to the total above.
