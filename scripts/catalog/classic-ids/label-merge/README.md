...
contract is [§ 4](/docs/catalog-driver.md#4-how-hd-reaches-gaia).
...
**The key is the DERIVED binding** ([The four sources, in precedence order](../../membership/binding/README.md#the-four-sources-in-precedence-order)).
Keyed on the frozen cell a label
names a source the record is no longer bound to, and 1,371 rows reach no
overlay entry where the derivation leaves 578 — mostly the bright end, whose
saturated 2-parameter sources are an identity statement rather than a
measurement.
...
ladder's gate ([§ 4](/docs/star-naming.md#4-canonical-designation-forms)). So the overlay's `flamsteed` cell is
...
designation-keyed route ([The designation constellation](../README.md#the-designation-constellation)).
...
index's `hdMap` / `hrMap` ([Star search](/src/client/typeahead/README.md#star-search)) and
extra `hd:` / `hr:` designations in its same-as class ([§ 4.1](/docs/sid.md#41-same-as-equivalence-graph)).
...
attribution the table declines to make ([Promoted-companion field inheritance](../../companions/README.md#promoted-companion-field-inheritance)).
...
- **Of the 9, five have a second Tycho entry both IV/25 and SIMBAD give the
  ...
  ([The additions](../../membership/README.md#the-additions)). Handing the number to a
  ...
  [§ 4.1](/docs/sid.md#41-same-as-equivalence-graph) refuse.
...
[§ 4](/docs/catalog-driver.md#4-how-hd-reaches-gaia). It fires on 29 cells; p Eridani is the case, the
...
[§ 4](/docs/catalog-driver.md#4-how-hd-reaches-gaia) names: review finding the CDS join wrong. It holds
...
which component the record is ([Which witness decides a close pair's HD](../../simbad/README.md#which-witness-decides-a-close-pairs-hd)).
They sit here rather than in a mechanism of their own
...
([Admission is not a promise of a record](../../membership/README.md#admission-is-not-a-promise-of-a-record)). Five
...
([Star search](/src/client/typeahead/README.md#star-search)); checked against the built
...
([The GJ fold stops at the component](../README.md#the-gj-fold-stops-at-the-component)). `glieseNumber` states
