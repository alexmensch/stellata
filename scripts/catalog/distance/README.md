...
| `tycho2` | No Gaia astrometry row AND no HIP2 row, but the record carries a TYC. Tycho-2's **mean** position (`ra_mdeg`/`de_mdeg`), which the catalogue states at J2000. | 43 |
...
| `tycho2`, `pflag='X'` rows | J1991.25 — no mean solution exists, so the observed `ra_icrs` is the only position the row has, and it solves to 1991.67 over the 133 measurable rows. Their PM comes from `pm-rescue/` instead, and advances this position like any other. 3 of the 43, pinned `directionTycho2FromIcrs` |
...
one of the 43 carries a HIP, a Gaia source_id or a proper name, so the other
...
0.149″ / 0.109″); the 273 Gaia rows are native J2016.0 and the advance is a
